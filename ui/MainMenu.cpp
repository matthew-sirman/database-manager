...
MainMenu::MainMenu(const std::filesystem::path &clientMetaFilePath,
                   QWidget *parent)
...
#ifdef _DEBUG
  pricing = new QLibrary("pricing_package_d.dll");
#else
  pricing = new QLibrary("pricing_package.dll");
#endif
  pricing->setLoadHints(QLibrary::ResolveAllSymbolsHint |
  ...
  for (int i = 0; i < 10; i++) {
    ...
    connect(changeTab, &QShortcut::activated, [this, i]() {
      ...
      j--;
      if (j < ui->mainTabs->count()) ui->mainTabs->setCurrentIndex(j);
    });
  }

  // std::cout << Qt::Key_1 + 1 << " " << Qt::Key_2 << " " << (Qt::Key_1 + 1
  // == Qt::Key_2) << std::endl; for (unsigned i = 0; i < 9; i++) {
  //     QShortcut* changeTab = new QShortcut(ui->mainTabs);
  ...
  try {
    ...
    QMessageBox::about(
        this, "Invalid Client Meta File",
        "Failed to load the client meta file. "
  ...
  if (clientMeta.find("keyPath") == clientMeta.end()) {
    QMessageBox::about(
        this, "Invalid Client Meta File",
        "Ensure that the client meta file contains "
  ...
  std::filesystem::path serverSignaturePath =
      clientMeta["serverSignaturePath"].get<std::string>();
  ...
  if (!std::filesystem::exists(keyPath / "client_key.pri") ||
      !std::filesystem::exists(keyPath / "client_key.pub")) {
    QMessageBox::about(this, "Generating Keys",
                       "Your client will shortly begin generating keys "
  ...
  nlohmann::json::iterator repeatTokenPathIter =
      clientMeta.find("repeatTokenPath");

  if (repeatTokenPathIter != clientMeta.end()) {
    std::filesystem::path repeatTokenPath =
        (*repeatTokenPathIter).get<std::string>();
    ...
    switch (client->connectWithToken(serverIP, serverPort, repeatToken)) {
      case Client::ConnectionStatus::NO_CONNECTION:
        QMessageBox::about(
            this, "Connection to Server Failed",
            "Failed to connect to the server. "
            "The application cannot be used without a connection to "
            "the server. Is the server running?");

        exit(0);
      case Client::ConnectionStatus::CREDS_EXCHANGE_FAILED:
        QMessageBox::about(
            this, "Connection to Server Failed",
            "Credentials exchange failed. "
      ...
      case Client::ConnectionStatus::INVALID_REPEAT_TOKEN:
        QMessageBox::about(
            this, "Invalid Token",
            "Your repeat login token has expired. Please log back in "
            "through Outlook.");
        ...
        if (QMessageBox::question(this, "Save Login Details",
                                  "Would you like to save login "
                                  "details for future use?") ==
            QMessageBox::Yes) {
          handler->setRepeatTokenResponseCallback(
              [this, repeatTokenPath](const uint256 &token) {
                std::ofstream repeatTokenFile(repeatTokenPath);
                repeatTokenFile.write((const char *)&token, sizeof(uint256));
                repeatTokenFile.close();
              });

          client->requestRepeatToken(
    ...
    if (QMessageBox::question(
            this, "Save Login Details",
            "Would you like to save login details for future use?") ==
        QMessageBox::Yes) {
      handler->setRepeatTokenResponseCallback(
          [this, clientMetaFilePath, clientMeta](const uint256 &token) mutable {
            std::string repeatTokenFilePath =
                (clientMetaFilePath / "repeat.tok").string();
            ...
            std::ofstream clientMetaFile(clientMetaFilePath /
                                         "clientMeta.json");
  ...
  connect(ui->searchResultsTable,
          SIGNAL(customContextMenuRequested(const QPoint &)), this,
          SLOT(handleSearchElementContextMenu(const QPoint &)));
  connect(
      ui->searchResultsTable, &QTableView::doubleClicked,
  ...
  connect(ui->mainTabs, SIGNAL(tabCloseRequested(int)), this,
          SLOT(closeTab(int)));
  connect(ui->drawingMenu_addDrawingAction, &QAction::triggered,
          [this]() { openAddDrawingTab(NextDrawing::DrawingType::AUTOMATIC); });
  connect(ui->drawingMenu_addManualDrawingAction, &QAction::triggered,
          [this]() { openAddDrawingTab(NextDrawing::DrawingType::MANUAL); });
  connect(this, SIGNAL(itemAddedToDrawingQueue()), this,
          SLOT(processDrawings()));
  if (client->hasFullAccess()) {
    QMenu *componentsMenu = ui->menubar->addMenu("Components");
    QAction *addNewAperture = new QAction("Add new Aperture");
    QAction *addNewMachine = new QAction("Add new Machine");
    QAction *addNewSideIron = new QAction("Add new Side Iron");
    QAction *addNewMaterial = new QAction("Add new Material");
    QAction *addNewStrap = new QAction("Add new Strap");
    componentsMenu->addActions({addNewAperture, addNewMachine, addNewSideIron,
                                addNewMaterial, addNewStrap});
    connect(addNewAperture, &QAction::triggered, [this]() {
      (new AddApertureWindow(client))->show();
      //(new AddComponentWindow<Aperture, float, float, unsigned, unsigned,
      //                        unsigned, ApertureShape, std::optional<Aperture>>(
      //     client, "Aperture",
      //     {"Width", "Length", "Base Width", "Base Length", "Quantity",
      //      "Aperture Shape", "Nibble Aperture"},
      //     this))
      //    ->show();
    });
    connect(addNewMachine, &QAction::triggered, [this]() {
      //(new AddComponentWindow<Machine, std::string, std::string>(client, "Machine", {"Manufacturer", "Model"}, this))
      //    ->show();
      (new AddMachineWindow(client))->show();
    });
    connect(addNewSideIron, &QAction::triggered, //[this]() {
      [this]() { 
      (new AddSideIronWindow(client))->show();
          //(new AddComponentWindow<
          //    SideIron, SideIronType, unsigned short, std::string, std::filesystem::path,
          //    bool, std::optional<float>, std::optional<unsigned>>(
          //    client, "Side Iron",
          //        {"Side Iron Type", "length", "Drawing Number", "Hyperlink",
          //          "Extraflex", "Price", "screws"},
          //         this))
          //        ->show();
      });
    connect(addNewMaterial, &QAction::triggered,
            [this]() { (new AddMaterialWindow(client))->show(); });
    connect(addNewStrap, &QAction::triggered, [this]() {
      //(new AddComponentWindow<Strap, Material, bool>(
      //     client, "Strap", {"Material", "Is Wear Tear Liner"}, this))
      //    ->show();
    });

    QMenu *pricingMenu = ui->menubar->addMenu("Prices");
    QAction *materialPricesAction = new QAction("Material Prices");
    QAction *stockSideIronPricesAction = new QAction("Stock Side Iron Prices");
    QAction *specificSideIronPricesAction =
        new QAction("Specific Side Iron Prices");
    QAction *extraPricesAction = new QAction("Extra Prices");
    QAction *labourTimesAction = new QAction("Labour Times");
    QAction *powderCoatingPricesAction = new QAction("Powder Coating Prices");
    pricingMenu->addActions({materialPricesAction, stockSideIronPricesAction,
                             specificSideIronPricesAction, extraPricesAction,
                             labourTimesAction, powderCoatingPricesAction});
    connect(materialPricesAction, &QAction::triggered,
            [this]() { (new MaterialPricingWindow(client))->show(); });
    connect(stockSideIronPricesAction, &QAction::triggered,
            [this]() { (new SideIronPricingWindow(client))->show(); });
    connect(specificSideIronPricesAction, &QAction::triggered,
            [this]() { (new SpecificSideIronPricingWindow(client))->show(); });
    connect(extraPricesAction, &QAction::triggered,
            [this]() { (new ExtraPricingWindow(client))->show(); });
    connect(labourTimesAction, &QAction::triggered,
            [this]() { (new LabourTimesWindow(client))->show(); });
    connect(powderCoatingPricesAction, &QAction::triggered,
            [this]() { (new PowderCoatingPricingWindow(client))->show(); });
  }

  handler->setAddComponentResponseCallback(
      [this](ComponentInsert::ComponentInsertResponse responseCode) {
        emit addComponentResponseReceived(responseCode);
      });

  connect(
      this,
      SIGNAL(addComponentResponseReceived(
          ComponentInsert::ComponentInsertResponse)),
      this,
      SLOT(insertComponentResponse(ComponentInsert::ComponentInsertResponse)));
  ...
  connect(ui->fileMenu_createBackupAction, &QAction::triggered, [this]() {
    ...
    QString backupName =
        QInputDialog::getText(this, "Database Backup", "Enter name for backup",
                              QLineEdit::Normal, dateString.str().c_str());

    if (backupName.isEmpty()) {
      QMessageBox::about(this, "Invalid Backup Name",
                         "Backup name must not be empty.");
      return;
    }
    if (backupName.contains(QRegularExpression("\\|/"))) {
      QMessageBox::about(this, "Invalid Backup Name",
                         "Backup name must not contain slashes.");
      return;
    }
  ...
  handler->setBackupResponseCallback(
      [this](DatabaseBackup::BackupResponse responseCode) {
  ...
  handler->setDrawingReceivedHandler([this](DrawingRequest &drawingRequest) {
    onReceiveDrawing(drawingRequest);
  });

  handler->setDrawingInsertResponseHandler(
      [this](DrawingInsert::InsertResponseCode responseType,
    ...
    switch (nextDrawing.drawingType) {
      case NextDrawing::DrawingType::AUTOMATIC:
        ...
        if (std::regex_match(nextAutomaticDrawingNumber, rx)) {
          nextAutomaticDrawingNumber =
              nextAutomaticDrawingNumber.insert(2, "0");
        }
        break;
  ...
  connect(
      this,
      SIGNAL(insertDrawingResponseReceived(DrawingInsert::InsertResponseCode,
                                           unsigned)),
      this,
      SLOT(insertDrawingResponse(DrawingInsert::InsertResponseCode, unsigned)));
...
void MainMenu::sendSourceTableRequests() const {
  ...
  sourceTable(RequestType::SOURCE_STRAPS_TABLE);
}
...
void MainMenu::setupComboboxSources() {
  static std::unordered_map<unsigned char, unsigned char> shapeOrder = {
      {1, 1}, {2, 4}, {3, 2}, {4, 3}, {5, 5}, {6, 0}};
  std::function<bool(const Aperture &, const Aperture &)> apertureComparator =
      [](const Aperture &a, const Aperture &b) {
  ...
  DrawingComponentManager<Aperture>::addCallback(
      [this]() { apertureSource.updateSource(); });
  DrawingComponentManager<ApertureShape>::addCallback(
      [this]() { apertureShapeSource.updateSource(); });
  DrawingComponentManager<Material>::addCallback([this]() {
  ...
  connect(ui->manufacturerSearchInput, &DynamicComboBox::currentTextChanged,
...
void MainMenu::setupValidators() {
  QRegularExpressionValidator *drawingNumberValidator =
      new QRegularExpressionValidator(QRegularExpression(
          "^([a-zA-Z]{1,2}[0-9]{0,2}[a-zA-Z]?|M[0-9]{0,}[a-zA-Z]?)$"));

  ui->drawingNumberSearchInput->setValidator(drawingNumberValidator);
  connect(ui->drawingNumberSearchInput, SIGNAL(textEdited(const QString &)),
          this, SLOT(capitaliseLineEdit(const QString &)));

  QRegularExpressionValidator *positionSearchValidator =
      new QRegularExpressionValidator(
          QRegularExpression("(^$)|(^[0-9]+([-][0-9]+)?$)|(^[Aa][Ll]{2}$)"));

  ui->positionSearchInput->setValidator(positionSearchValidator);
  connect(ui->positionSearchInput, SIGNAL(textEdited(const QString &)), this,
...
void MainMenu::setupActivators() {
  ...
  ui->manufacturerLabel->addActivationCallback(
      [this](bool active) { ui->manufacturerSearchInput->clearEditText(); });
  ui->modelLabel->addTarget(ui->modelSearchInput);
  ui->modelLabel->addActivationCallback(
      [this](bool active) { ui->modelSearchInput->clearEditText(); });
...
void MainMenu::setupSearchResultsTable() {
  ...
  handler->setPopulateResultsModel(
      std::bind(&DrawingSearchResultsModel::sourceDataFromBuffer,
                searchResultsModel, std::placeholders::_1));
  ui->searchResultsTable->setModel(searchResultsModel);

  ui->searchResultsTable->horizontalHeader()->setStretchLastSection(true);
  ui->searchResultsTable->horizontalHeader()->setSectionResizeMode(
...
void MainMenu::connectToServerWithJWT(const std::string &serverIP,
                                      unsigned serverPort) {
  switch (
      client->connectToServer(serverIP, serverPort, [](const std::string &url) {
    ...
    case Client::ConnectionStatus::NO_CONNECTION:
      QMessageBox::about(
          this, "Connection to Server Failed",
          "Failed to connect to the server. "
          "The application cannot be used without a connection to the "
          "server. Is the server running?");
    ...
    case Client::ConnectionStatus::INVALID_JWT:
      QMessageBox::about(this, "Invalid Login",
                         "Your login was denied by the server.");

      exit(0);
    case Client::ConnectionStatus::INVALID_REPEAT_TOKEN:
      QMessageBox::about(this, "Invalid Repeat Token",
                         "Your Repeat Token was denied by the server.");
...
void MainMenu::searchButtonPressed() {
  ...
  if (ui->productTypeLabel->active()) {
    query.productType = DrawingComponentManager<Product>::getComponentByHandle(
        ui->productTypeSearchInput->currentData().toInt());
  }
  ...
  if (ui->apertureLabel->active()) {
    query.aperture = DrawingComponentManager<Aperture>::getComponentByHandle(
        ui->apertureSearchInput->currentData().toInt());
  }
  if (ui->thicknessSearchLabel->active()) {
    query.topThickness =
        DrawingComponentManager<Material>::getComponentByHandle(
            ui->thicknessSearchInput->currentData().toInt());
  }
  if (ui->backingThicknessSearchLabel->active()) {
    query.bottomThickness =
        DrawingComponentManager<Material>::getComponentByHandle(
            ui->backingThicknessSearchInput->currentData().toInt());
  }
  if (ui->startDateLabel->active()) {
    QDate date = ui->startDateSearchInput->date();
    query.dateRange = {{(unsigned short)date.year(),
                        (unsigned char)date.month(), (unsigned char)date.day()},
                       {9999, 12, 31}};
  }
  if (ui->endDateLabel->active()) {
    ...
    if (query.dateRange == std::nullopt) {
      query.dateRange = {
          {0, 1, 1},
          {(unsigned short)date.year(), (unsigned char)date.month(),
           (unsigned char)date.day()}};
    } else {
      query.dateRange->upperBound = {(unsigned short)date.year(),
  ...
  if (ui->sideIronTypeLabel->active()) {
    query.sideIronType =
        (SideIronType)(ui->sideIronTypeSearchInput->currentIndex());
  }
  ...
  if (ui->sidelapAttachmentLabel->active()) {
    query.sidelapAttachment =
        (LapAttachment)ui->sidelapAttachmentSearchInput->currentIndex();
  }
  if (ui->overlapAttachmentLabel->active()) {
    query.overlapAttachment =
        (LapAttachment)ui->overlapAttachmentSearchInput->currentIndex();
  }
  if (ui->manufacturerLabel->active()) {
    query.manufacturer =
        ui->manufacturerSearchInput->currentText().toStdString();
  }
  if (ui->modelLabel->active()) {
    query.machine = DrawingComponentManager<Machine>::getComponentByHandle(
        ui->modelSearchInput->currentData().toInt());
  }
  ...
  if (ui->deckLabel->active()) {
    query.machineDeck =
        DrawingComponentManager<MachineDeck>::getComponentByHandle(
            ui->deckSearchInput->currentData().toInt());
  }
...
void MainMenu::handleSearchElementContextMenu(const QPoint &pos) {
  ...
  if (item.isValid()) {
    if (item.column() == 0) {
      ...
      menu->addAction(
          ("Open drawing " + summary.drawingNumber + " in new tab").c_str(),
...
void MainMenu::openAddDrawingTab(NextDrawing::DrawingType type) {
  ...
  switch (type) {
    case NextDrawing::DrawingType::AUTOMATIC:
      addDrawingPage = new AddDrawingPageWidget(nextAutomaticDrawingNumber,
                                                true, ui->mainTabs);
      addDrawingPage->setUserEmail(clientEmailAddress);
      break;
    case NextDrawing::DrawingType::MANUAL:
      addDrawingPage = new AddDrawingPageWidget(nextManualDrawingNumber, false,
                                                ui->mainTabs);
      addDrawingPage->setUserEmail(clientEmailAddress);
      break;
    default:
  ...
  addDrawingPage->setConfirmationCallback(
      [this](const Drawing &drawing, bool force) {
  ...
  while (!drawingReceivedQueue.empty()) {
    ...
    switch (drawingResponseActions[request->responseEchoCode]) {
      case OPEN_DRAWING_VIEW_TAB:
        if (request->drawingData->loadWarning(
                Drawing::LoadWarning::LOAD_FAILED)) {
          QMessageBox::about(this, "Drawing Load Failed",
                             "Attempt to load this drawing from "
                             "the database failed.");
        } else {
          DrawingViewWidget *drawingView =
              new DrawingViewWidget(request->drawingData.value(), ui->mainTabs);
          Drawing &drawing = request->drawingData.value();

          drawingView->setChangeDrawingCallback(
              [this,
               drawing](AddDrawingPageWidget::AddDrawingMode mode) mutable {
                bool automatic;
                ...
                AddDrawingPageWidget *addDrawingPage = new AddDrawingPageWidget(
                    drawing, mode, automatic, ui->mainTabs);
                addDrawingPage->setUserEmail(clientEmailAddress);
                addDrawingPage->setConfirmationCallback(
                    [this](const Drawing &drawing, bool force) {
                      std::unique_ptr<DrawingInsert> insert(
                          new DrawingInsert());
                      insert->drawingData = drawing;
          ...
          ui->mainTabs->addTab(
              drawingView,
              tr((request->drawingData->drawingNumber() + " Details").c_str()));
...
void MainMenu::insertDrawingResponse(
    DrawingInsert::InsertResponseCode responseType, unsigned responseCode) {
  switch (responseType) {
    case DrawingInsert::NONE:
      break;
    case DrawingInsert::SUCCESS:
      QMessageBox::about(this, "Insert Drawing",
                         "Drawing successfully added to database.");
      drawingInserts.erase(responseCode);
      break;
    case DrawingInsert::FAILED:
      QMessageBox::about(this, "Insert Drawing",
                         "The attempt to add the drawing to the database "
                         "was unsuccessful.");
      drawingInserts.erase(responseCode);
      break;
    case DrawingInsert::DRAWING_EXISTS:
      QMessageBox::StandardButton questionResponse =
          QMessageBox::question(this, "Insert Drawing",
                                "This drawing already exists in the "
                                "database. Would you like "
                                "to update it?");
...
void MainMenu::insertComponentResponse(
    ComponentInsert::ComponentInsertResponse responseCode) {
  switch (responseCode) {
    case ComponentInsert::ComponentInsertResponse::SUCCESS:
      QMessageBox::about(this, "Add Component",
                         "Component successfully added to database.");
      break;
    case ComponentInsert::ComponentInsertResponse::FAILED:
      QMessageBox::about(this, "Add Component",
                         "There was an error while trying to add "
...
void MainMenu::backupResponse(DatabaseBackup::BackupResponse responseCode) {
  switch (responseCode) {
    case DatabaseBackup::BackupResponse::SUCCESS:
      QMessageBox::about(this, "Database Backup",
                         "Backup created successfully.");
      break;
    case DatabaseBackup::BackupResponse::FAILED:
      QMessageBox::about(
          this, "Database Backup",
          "There was an error while trying to create the backup.");
      break;
  }
}