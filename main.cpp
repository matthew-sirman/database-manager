...
//extern template class __declspec(dllimport) DrawingComponentManager<Aperture>;
//extern template class __declspec(dllimport)
//    DrawingComponentManager<ApertureShape>;
//extern template class __declspec(dllimport)
//    DrawingComponentManager<BackingStrip>;
//extern template class __declspec(dllimport) DrawingComponentManager<ExtraPrice>;
//extern template class __declspec(dllimport) DrawingComponentManager<LabourTime>;
//extern template class __declspec(dllimport) DrawingComponentManager<Machine>;
//extern template class __declspec(dllimport)
//    DrawingComponentManager<MachineDeck>;
//extern template class __declspec(dllimport) DrawingComponentManager<Material>;
//extern template class __declspec(dllimport)
//    DrawingComponentManager<PowderCoatingPrice>;
//extern template class __declspec(dllimport) DrawingComponentManager<Product>;
//extern template class __declspec(dllimport) DrawingComponentManager<SideIron>;
//extern template class __declspec(dllimport)
//    DrawingComponentManager<SideIronPrice>;
//extern template class __declspec(dllimport) DrawingComponentManager<Strap>;
//extern template class __declspec(dllimport)
//    ExtraPriceManager<ExtraPriceType::SIDE_IRON_NUTS>;
//extern template class __declspec(dllimport)
//    ExtraPriceManager<ExtraPriceType::SIDE_IRON_SCREWS>;
//extern template class __declspec(dllimport)
//    ExtraPriceManager<ExtraPriceType::TACKYBACK_GLUE>;
//extern template class __declspec(dllimport)
//    ExtraPriceManager<ExtraPriceType::LABOUR>;
//extern template class __declspec(dllimport)
//    ExtraPriceManager<ExtraPriceType::PRIMER>;
//extern template class __declspec(dllimport)
...
void setupServerKeys(std::filesystem::path metaFilePath) {
  ...
  std::filesystem::path databasePasswordPath =
      meta["databasePasswordPath"].get<std::string>();
  ...
  lockPrivateKey(serverKeyPair.privateKey,
                 keyPath / "server/server_key_root.pri", pwHash);
  writePublicKey(serverKeyPair.publicKey, keyPath / "server/server_key.pub");
  lockPrivateKey(digitalSignatureKeyPair.privateKey,
                 keyPath / "signature/signature_root.pri", pwHash);
  ...
  while (true) {
    ...
    if (response == "y" || response == "Y" || response == "yes") {
      ...
      lockPrivateKey(serverKeyPair.privateKey,
                     keyPath / ("server/server_key_" + uname + ".pri"), pwHash);
      lockPrivateKey(digitalSignatureKeyPair.privateKey,
...
void addUser(const std::string &newUser, std::filesystem::path metaFilePath,
  ...
  if (!std::filesystem::exists(keyPath /
                               ("server/server_key_" + authUser + ".pri")) ||
  ...
  if (serverKeyPair.privateKey.n == serverKeyPair.publicKey.n &&
    ...
    lockPrivateKey(serverKeyPair.privateKey,
                   keyPath / ("server/server_key_" + newUser + ".pri"), pwHash);
    lockPrivateKey(digitalSignatureKeyPair.privateKey,
...
void runServer(std::filesystem::path metaFilePath, const std::string &user,
  ...
  if (meta.find("logFile") != meta.end()) {
    logStream =
        new std::ofstream(meta["logFile"].get<std::string>(), std::ios::app);
  ...
  if (meta.find("errorFile") != meta.end()) {
    errStream =
        new std::ofstream(meta["errorFile"].get<std::string>(), std::ios::app);
  ...
  Logger::setChangelogStream(changelogStream);

  s.setLoggingStream(errStream);
  ...
  if (!dev) {
    s.connectToDatabaseServer("screen_mat_database", "db-server-user",
                              databasePassword, "scs.local");
  } else {
  ...
  // Start running the server - this will enter a loop and return when the
  // server is closed
  s.startServer();
...
int runClient(const std::filesystem::path &clientMetaFile, int argc,
              char *argv[]) {
...
void test() {
  QMessageBox::information(nullptr, "Popup",
                           "This is a popup from a non-GUI thread.");
}
...
int main(int argc, char *argv[]) {
...
#endif
  // #ifdef _DEBUG
  //   HMODULE hModule = LoadLibrary(TEXT("pricing_package_d.dll"));
  // #else
  //   HMODULE hModule = LoadLibrary(TEXT("pricing_package.dll"));
  // #endif
  //   if (hModule == NULL) {
  //     // Handle error, the DLL could not be loaded.
  //     exit(-1);
  //   }
  ...
  if (argc > 1) {
    for (int i = 1; i < argc; i++) {
      // If the user users the argument "-h" or "--help" set the mode to help
      // and break instantly (they want help; not to use the application in any
      // way)
      ...
      if (strcmp(argv[i], "--add-user") == 0) {
        if (mode == NO_MODE) {
          ...
          if (!std::regex_match(newUser, USERNAME_CHECK)) {
            std::cerr
                << "Invalid username for add user. Use lowercase letters only."
                << std::endl;
            goto error;
          }
        } else {
      ...
      // If they provided a specific username to load the key files with, use
      // that. Default to root.
