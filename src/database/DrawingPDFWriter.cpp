...
bool DrawingPDFWriter::createPDF(const std::filesystem::path &pdfFilePath,
  ...
  QFontDatabase::addApplicationFont(":/fonts/SEGOEUIL.ttf");

  //QFont font("segoe ui");
  ////painter.save();
  //font.setPointSize(400);  // Adjust as needed
  ...
  drawRubberScreenCloth(
      painter, svgTemplateRenderer.boundsOnElement("drawing_target_region"),
      drawing);

  return true;
}

void DrawingPDFWriter::drawStandardTemplate(
    QPainter &painter, QSvgRenderer &svgTemplateRenderer) const {
...
void DrawingPDFWriter::drawLabelAndField(QPainter &painter, double left,
                                         double &top, const QString &label,
                                         double labelWidth,
                                         const QString &field,
                                         double fieldWidth, double hOffset,
  ...
  QRectF labelBounds =
      QFontMetrics(painter.font())
          .boundingRect(QRect(left, top, labelWidth, 0)
                            .adjusted(hOffset, vOffset, -hOffset, -vOffset),
                        Qt::TextWordWrap, label);
  QRectF fieldBounds =
      QFontMetrics(painter.font())
          .boundingRect(QRect(left + labelWidth, top, fieldWidth, 0)
                            .adjusted(hOffset, vOffset, -hOffset, -vOffset),
...
void DrawingPDFWriter::drawTextDetails(
    QPainter &painter, QSvgRenderer &svgTemplateRenderer,
    const Drawing &drawing, const std::string &drawnByInitials) const {
  QRect viewport = painter.viewport();
  painter.save();
  ...
  QRectF generalDetailsBox = svgTemplateRenderer.boundsOnElement(
             "general_details_target_region"),
         manufacturingDetailsBox = svgTemplateRenderer.boundsOnElement(
             "manufacturing_details_target_region");
  ...
  drawLabelAndField(painter, generalDetailsBox.left(), currentVPos, labelText,
                    labelWidth, fieldText, fieldWidth, horizontalOffset,
                    verticalOffset);
  ...
  if (drawing.product().productName != "Rubber Modules and Panels") {
    ...
    if (drawing.hasOverlaps() || drawing.hasSidelaps()) {
      std::optional<Drawing::Lap> leftOverlap = drawing.overlap(Drawing::LEFT),
                                  rightOverlap =
                                      drawing.overlap(Drawing::RIGHT),
                                  leftSidelap = drawing.sidelap(Drawing::LEFT),
                                  rightSidelap =
                                      drawing.sidelap(Drawing::RIGHT);
      ...
      if (leftOverlap.has_value()) {
        ...
        if (leftOverlap->attachmentType == LapAttachment::BONDED) {
          lapString << " (" << leftOverlap->material().materialName << " "
                    << leftOverlap->material().materialName << "mm)";
        }
      ...
      if (rightOverlap.has_value()) {
        ...
        if (rightOverlap->attachmentType == LapAttachment::BONDED) {
          lapString << " (" << rightOverlap->material().materialName << " "
                    << rightOverlap->material().thickness << "mm)";
        }
        lapStrings.push_back(lapString.str());
      }
      if (leftSidelap.has_value()) {
        ...
        if (leftSidelap->attachmentType == LapAttachment::BONDED) {
          lapString << " (" << leftSidelap->material().materialName << " "
                    << leftSidelap->material().thickness << "mm)";
        }
        lapStrings.push_back(lapString.str());
      }
      if (rightSidelap.has_value()) {
        ...
        if (rightSidelap->attachmentType == LapAttachment::BONDED) {
          lapString << " (" << rightSidelap->material().materialName << " "
                    << rightSidelap->material().thickness << "mm)";
        }
        lapStrings.push_back(lapString.str());
      }

      for (std::vector<std::string>::const_iterator it = lapStrings.begin();
           it != lapStrings.end(); it++) {
        sideOverlapsText << *it;
  ...
  if (productName == "Rubber Screen Cloth") {
    if (drawing.material(Drawing::TOP)->thickness >= 15) {
      ...
      drawLabelAndField(painter, generalDetailsBox.left(), currentVPos,
                        labelText, labelWidth, fieldText, fieldWidth,
      ...
      labelText = "Backing Strips";
      fieldText = drawing.hasBackingStrips()
                      ? drawing.backingStrip()->backingStripName().c_str()
                      : "No";
      drawLabelAndField(painter, generalDetailsBox.left(), currentVPos,
                        labelText, labelWidth, fieldText, fieldWidth,
                        horizontalOffset, verticalOffset);
  ...
  if (drawing.numberOfImpactPads()) {
    ...
    for (unsigned i = 0; i < pads.size(); i++) {
      ...
      impactPadFieldText << pad.width << "x" << pad.length << " at "
                         << "(" << pad.pos.x << ", " << pad.pos.y << ") ";
      impactPadFieldText << mat.thickness << "mm " << mat.materialName << ", "
                         << pad.aperture().apertureName();
  ...
  if (drawing.numberOfBlankSpaces()) {
    ...
    for (unsigned i = 0; i < spaces.size(); i++) {
      ...
      blankSpaceFieldText << space.width << "x" << space.length << " at "
  ...
  if (drawing.numberOfExtraApertures()) {
    ...
    for (unsigned i = 0; i < apertures.size(); i++) {
      ...
      ExtraAperturesFieldText
          << aperture.aperture().apertureName() << " Aperture across "
          << aperture.width << "x" << aperture.length << " at "
  ...
  if (drawing.numberOfDamBars()) {
    ...
    for (unsigned i = 0; i < bars.size(); i++) {
      ...
      damBarTextField << bar.width << "x" << bar.length << " at "
                      << "(" << bar.pos.x << ", " << bar.pos.y << ") ";
  ...
  if (drawing.numberOfCentreHoles()) {
    ...
    std::string shape =
        DrawingComponentManager<ApertureShape>::findComponentByID(
            aperture.apertureShapeID)
            .shape;
  ...
  if (drawing.numberOfDeflectors()) {
    ...
    deflectorsFieldText << deflectors.front().size << "mm " << mat.thickness
                        << "mm " << mat.materialName << " at ";

    for (unsigned i = 0; i < deflectors.size(); i++) {
      deflectorsFieldText << "(" << deflectors[i].pos.x << ", "
                          << deflectors[i].pos.y << ")";
  ...
  if (drawing.numberOfDivertors()) {
    ...
    divertorsFieldText << divertors.front().width << "x"
                       << divertors.front().length << " " << mat.thickness
                       << "mm " << mat.materialName << " at ";
  ...
  QRectF fieldDrawingNumberBox = DrawingPDFWriter::adjustRect(
             svgTemplateRenderer.boundsOnElement("field_drawing_number")),
         fieldTitleBox = DrawingPDFWriter::adjustRect(
             svgTemplateRenderer.boundsOnElement("field_title")),
         fieldDateBox = DrawingPDFWriter::adjustRect(
             svgTemplateRenderer.boundsOnElement("field_date")),
         fieldDrawnByBox = DrawingPDFWriter::adjustRect(
             svgTemplateRenderer.boundsOnElement("field_drawn_by")),
         fieldSCSLogoBox = DrawingPDFWriter::adjustRect(
             svgTemplateRenderer.boundsOnElement("field_scs_logo")),
         fieldSCSDetailsBox = DrawingPDFWriter::adjustRect(
             svgTemplateRenderer.boundsOnElement("field_scs_details"));
  ...
  if (drawing.product().productName != "Bivitec" &&
    ...
    title << " " + to_str(drawing.numberOfBars()) + " Support Bar" +
                 (drawing.numberOfBars() == 1 ? "" : "s");
  }
  ...
  details << "ANY REPRODUCTION IN PART OR AS A WHOLE WITHOUT THE WRITTEN "
             "PERMISSION ";
  ...
  painter.drawText(fieldSCSDetailsBox, details.str().c_str(),
                   centreAlignedText);
  ...
  painter.drawText(fieldDrawingNumberBox, drawing.drawingNumber().c_str(),
                   centreAlignedText);

  painter.restore();
}

void DrawingPDFWriter::drawRubberScreenCloth(QPainter &painter,
                                             QRectF drawingRegion,
                                             const Drawing &drawing) const {
  const double maxDimensionPercentage = 0.9;
  const double defaultHorizontalBarSize = 45;
  const double dimensionSpacingHeight = 0.7, dimensionBarHeight = 0.8,
               dimensionInnerSpacingHeight = 0.9;
  const double dimensionHorizontalLapOffset = 0.3,
               dimensionVerticalLapOffset = 0.3;
  const double shortDimensionLineSize = 0.02, longDimensionLineSize = 0.04;
  const double mainDimensionLineSize = 0.06;

  QPen dashDotPen = QPen(QBrush(Qt::black), 5, Qt::DashDotLine);
  dashDotPen.setDashPattern({96, 48, 4, 48});

  QPen dashPen = QPen(QBrush(Qt::black), 5, Qt::DashLine);
  dashPen.setDashPattern({96, 48});

  QPen smallDashPen = QPen(QBrush(Qt::black), 5, Qt::DashLine);
  ...
  double regionWidth = drawingRegion.width(),
         regionHeight = drawingRegion.height();
  ...
  struct {
    ...
    inline double total() const { return rLeft + rCentre + rRight; }
  } widthDim, lengthDim;
  ...
  if (widthDim.total() / regionWidth > lengthDim.total() / regionHeight) {
    ...
    pLength = maxDimensionPercentage * ((lengthDim.total() / regionHeight) /
                                        (widthDim.total() / regionWidth));
  } else {
    pLength = maxDimensionPercentage;
    pWidth = maxDimensionPercentage * ((widthDim.total() / regionWidth) /
                                       (lengthDim.total() / regionHeight));
  }

  QRectF matBoundingRegion;
  matBoundingRegion.setTopLeft(QPointF(
      (0.5 - pWidth * (0.5 - widthDim.rLeft / widthDim.total())) * regionWidth +
          drawingRegion.left(),
      (0.5 - pLength * (0.5 - lengthDim.rLeft / lengthDim.total())) *
              regionHeight +
          drawingRegion.top()));
  matBoundingRegion.setBottomRight(
      QPointF((0.5 + pWidth * (0.5 - widthDim.rRight / widthDim.total())) *
                      regionWidth +
                  drawingRegion.left(),
              (0.5 + pLength * (0.5 - lengthDim.rRight / lengthDim.total())) *
                      regionHeight +
                  drawingRegion.top()));
  ...
  QLineF leftWidthExtender(
      ...
      QPointF(matBoundingRegion.left(),
              (0.5 * (1.0 - pLength) - mainDimensionLineSize) * regionHeight +
                  drawingRegion.top()));
  ...
  if (drawing.sideIron(Drawing::LEFT).type == SideIronType::B ||
      drawing.sideIron(Drawing::RIGHT).type == SideIronType::B) {
    drawArrow(
        painter, leftWidthExtender.center(), rightWidthExtender.center(),
        (to_str(
             drawing.width() +
             60 *
                 ((drawing.sideIron(Drawing::LEFT).type == SideIronType::B) +
                  (drawing.sideIron(Drawing::RIGHT).type == SideIronType::B))) +
         " o/h\n" + to_str(drawing.width()))
            .c_str(),
        DOUBLE_HEADED, false, QPen(), QPen(), 50.0, 30.0, 2);
  } else {
    drawArrow(painter, leftWidthExtender.center(), rightWidthExtender.center(),
              (to_str(drawing.width())).c_str(), DOUBLE_HEADED);
  }

  QLineF topLengthExtender(
      QPointF((0.5 * (1.0 - pWidth)) * regionWidth + drawingRegion.left(),
              matBoundingRegion.top()),
      QPointF((0.5 * (1.0 - pWidth) - mainDimensionLineSize) * regionWidth +
                  drawingRegion.left(),
              matBoundingRegion.top()));
  ...
  if (drawing.sideIronFixedEnd(Drawing::LEFT).has_value() ||
      drawing.sideIronFixedEnd(Drawing::RIGHT).has_value()) {
    needed = true;
    ...
    QRectF leftSideIronLowerLabel =
        QFontMetricsF(painter.font())
            .boundingRect(leftSideIronLowerText.str().c_str());
    leftSideIronLowerLabel.moveBottomLeft(drawingRegion.bottomLeft());
    leftSideIronLowerLabel.adjust(
        (bottomLengthExtender.center() - leftSideIronLowerLabel.center()).x(),
        QFontMetricsF(painter.font()).height(),
        (bottomLengthExtender.center() - leftSideIronLowerLabel.center()).x(),
        QFontMetricsF(painter.font()).height());

    painter.drawText(leftSideIronLowerLabel,
                     Qt::AlignHCenter | Qt::AlignVCenter,
                     leftSideIronLowerText.str().c_str());
    ...
    QRectF rightSideIronLowerLabel =
        QFontMetricsF(painter.font())
            .boundingRect(rightSideIronLowerText.str().c_str());
    rightSideIronLowerLabel.moveBottomRight(drawingRegion.bottomRight());
    rightSideIronLowerLabel.adjust(
        -(rightSideIronLowerLabel.center() - rightWidthExtender.center()).x(),
        QFontMetricsF(painter.font()).height(),
        -(rightSideIronLowerLabel.center() - rightWidthExtender.center()).x(),
        QFontMetricsF(painter.font()).height());

    painter.drawText(rightSideIronLowerLabel,
                     Qt::AlignHCenter | Qt::AlignVCenter,
                     rightSideIronLowerText.str().c_str());
  }

  if (drawing.sideIron(Drawing::LEFT).type !=
          drawing.sideIron(Drawing::RIGHT).type ||
    ...
    QRectF leftSideIronLabel =
        QFontMetricsF(painter.font())
            .boundingRect(leftSideIronText.str().c_str());
    leftSideIronLabel.moveBottomLeft(drawingRegion.bottomLeft());
    leftSideIronLabel.adjust(
        (bottomLengthExtender.center() - leftSideIronLabel.center()).x(), 0,
        (bottomLengthExtender.center() - leftSideIronLabel.center()).x(), 0);

    drawText(painter, drawingRegion.bottomLeft(),
             leftSideIronText.str().c_str());
    // painter.drawText(leftSideIronLabel, Qt::AlignHCenter | Qt::AlignVCenter,
    // leftSideIronText.str().c_str());
    ...
    QRectF rightSideIronLabel =
        QFontMetricsF(painter.font())
            .boundingRect(rightSideIronText.str().c_str());
    rightSideIronLabel.moveBottomRight(drawingRegion.bottomRight());
    rightSideIronLabel.adjust(
        -(rightSideIronLabel.center() - rightWidthExtender.center()).x(), 0,
        -(rightSideIronLabel.center() - rightWidthExtender.center()).x(), 0);

    drawText(painter, drawingRegion.bottomRight(),
             rightSideIronText.str().c_str());
    // painter.drawText(rightSideIronLabel, Qt::AlignHCenter | Qt::AlignVCenter,
    //                  rightSideIronText.str().c_str());
  }

  drawArrow(painter, topLengthExtender.center(), bottomLengthExtender.center(),
            to_str(drawing.length()).c_str(), DOUBLE_HEADED, true);

  if (leftLap.has_value()) {
    QRectF leftLapRegion;
    leftLapRegion.setLeft((0.5 * (1.0 - pWidth)) * regionWidth +
                          drawingRegion.left());
    ...
    QPointF lapLeft(leftLapRegion.left(),
                    matBoundingRegion.height() * dimensionVerticalLapOffset +
                        matBoundingRegion.top());
    ...
    drawArrow(
        painter,
        QPointF(lapLeft.x() - longDimensionLineSize * regionWidth, lapLeft.y()),
        lapLeft, lapString.c_str());
    drawArrow(painter,
              QPointF(lapRight.x() + shortDimensionLineSize * regionWidth,
                      lapRight.y()),
              lapRight);
  }
  if (rightLap.has_value()) {
    QRectF rightLapRegion;
    rightLapRegion.setRight((0.5 * (1.0 + pWidth)) * regionWidth +
                            drawingRegion.left());
    ...
    drawArrow(painter,
              QPointF(lapLeft.x() - shortDimensionLineSize * regionWidth,
                      lapLeft.y()),
              lapLeft);
    drawArrow(painter,
              QPointF(lapRight.x() + longDimensionLineSize * regionWidth,
                      lapRight.y()),
              lapRight, lapString.c_str(), SINGLE_HEADED, true);
  }
  if (topLap.has_value()) {
    QRectF topLapRegion;
    topLapRegion.setTop((0.5 * (1.0 - pLength)) * regionHeight +
                        drawingRegion.top());
    ...
    QPointF lapTop(matBoundingRegion.width() * dimensionHorizontalLapOffset +
                       matBoundingRegion.left(),
                   topLapRegion.top());
    QPointF lapBottom(matBoundingRegion.width() * dimensionHorizontalLapOffset +
                          matBoundingRegion.left(),
                      topLapRegion.bottom());
    ...
    drawArrow(
        painter,
        QPointF(lapTop.x(), lapTop.y() - longDimensionLineSize * regionHeight),
        lapTop, lapString.c_str());
    drawArrow(painter,
              QPointF(lapBottom.x(),
                      lapBottom.y() + shortDimensionLineSize * regionHeight),
              lapBottom);
  }
  if (bottomLap.has_value()) {
    QRectF bottomLapRegion;
    bottomLapRegion.setBottom((0.5 * (1.0 + pLength)) * regionHeight +
                              drawingRegion.top());
    ...
    drawArrow(
        painter,
        QPointF(lapTop.x(), lapTop.y() - shortDimensionLineSize * regionHeight),
        lapTop);
    drawArrow(painter,
              QPointF(lapBottom.x(),
                      lapBottom.y() + longDimensionLineSize * regionHeight),
              lapBottom, lapString.c_str());
  }
  ...
  for (unsigned bar = 0; bar < drawing.numberOfBars(); bar++) {
    ...
    barCentreDividerLine.setP1(QPointF(
        (currentMatPosition / widthDim.rCentre) * matBoundingRegion.width() +
            matBoundingRegion.left(),
        (defaultHorizontalBarSize / lengthDim.rCentre) *
                matBoundingRegion.height() +
            matBoundingRegion.top()));
    barCentreDividerLine.setP2(QPointF(
        (currentMatPosition / widthDim.rCentre) * matBoundingRegion.width() +
            matBoundingRegion.left(),
        (1 - (defaultHorizontalBarSize / lengthDim.rCentre)) *
                matBoundingRegion.height() +
            matBoundingRegion.top()));
  ...
  double apertureRegionTop = (defaultHorizontalBarSize / lengthDim.rCentre) *
                                 matBoundingRegion.height() +
                             matBoundingRegion.top(),
  ...
  for (unsigned apertureRegion = 0;
       apertureRegion < apertureRegionEndpoints.size() / 2; apertureRegion++) {
    double start = apertureRegionEndpoints[2 * apertureRegion];
    double end = apertureRegionEndpoints[2 * apertureRegion + 1];

    QRectF region;
    region.setTopLeft(
        QPointF((start / widthDim.rCentre) * matBoundingRegion.width() +
                    matBoundingRegion.left(),
                apertureRegionTop));
    region.setBottomRight(
        QPointF((end / widthDim.rCentre) * matBoundingRegion.width() +
                    matBoundingRegion.left(),
                apertureRegionBottom));
  ...
  for (unsigned spacingDimension = 0;
       spacingDimension <= drawing.numberOfBars(); spacingDimension++) {
    double nextSpacingPosition =
        spacingPosition + drawing.barSpacing(spacingDimension);

    if (drawing.numberOfBars() != 0) {
      drawArrow(painter,
                QPointF((spacingPosition / widthDim.rCentre) *
                                matBoundingRegion.width() +
                            matBoundingRegion.left(),
                        matBoundingRegion.height() * dimensionSpacingHeight +
                            matBoundingRegion.top()),
                QPointF((nextSpacingPosition / widthDim.rCentre) *
                                matBoundingRegion.width() +
                            matBoundingRegion.left(),
                        matBoundingRegion.height() * dimensionSpacingHeight +
                            matBoundingRegion.top()),
                to_str(drawing.barSpacing(spacingDimension)).c_str(),
                DOUBLE_HEADED);
      if (spacingDimension != drawing.numberOfBars()) {
        barWidthTotal += drawing.barSpacing(spacingDimension);
        QString runningTotal =
            QString::number(((int)(barWidthTotal * 10)) / 10.0f);
        QPointF topCenter((nextSpacingPosition / widthDim.rCentre) *
                                  matBoundingRegion.width() +
                              matBoundingRegion.left(),
                          (0.5 * (1.0 + pLength)) * regionHeight +
                              drawingRegion.top() +
                              longDimensionLineSize * regionHeight);
        drawText(painter, topCenter, runningTotal, 1,
                 [](QRectF &rect, const QPointF &point) {
    ...
    double leftInnerSpacing = apertureRegionEndpoints[2 * spacingDimension],
           rightInnerSpacing =
               apertureRegionEndpoints[2 * spacingDimension + 1];

    drawArrow(
        painter,
        QPointF(
            (leftInnerSpacing / widthDim.rCentre) * matBoundingRegion.width() +
                matBoundingRegion.left(),
            apertureRegionHeight * dimensionInnerSpacingHeight +
                apertureRegionTop),
        QPointF(
            (rightInnerSpacing / widthDim.rCentre) * matBoundingRegion.width() +
                matBoundingRegion.left(),
            apertureRegionHeight * dimensionInnerSpacingHeight +
                apertureRegionTop),
  ...
  for (unsigned bar = 0; bar < drawing.numberOfBars() + 2; bar++) {
    ...
    QPointF barRight(
        (barEndpoints[2 * bar + 1] / widthDim.rCentre) *
                matBoundingRegion.width() +
            matBoundingRegion.left(),
        apertureRegionHeight * dimensionBarHeight + apertureRegionTop);

    drawArrow(painter,
              QPointF(barLeft.x() - shortDimensionLineSize * regionWidth,
                      barLeft.y()),
              barLeft);
    drawArrow(painter,
              QPointF(barRight.x() + longDimensionLineSize * regionWidth,
                      barRight.y()),
              barRight, to_str(drawing.barWidth(bar)).c_str(), SINGLE_HEADED,
              true);
  }
  ...
  for (const Drawing::ImpactPad &pad : drawing.impactPads()) {
    QRectF padRegion(
        QPointF((pad.pos.x / widthDim.rCentre) * matBoundingRegion.width() +
                    matBoundingRegion.left(),
                (pad.pos.y / lengthDim.rCentre) * matBoundingRegion.height() +
                    matBoundingRegion.top()),
  ...
  for (const Drawing::BlankSpace &space : drawing.blankSpaces()) {
    QRectF padRegion(
        QPointF((space.pos.x / widthDim.rCentre) * matBoundingRegion.width() +
                    matBoundingRegion.left(),
                (space.pos.y / lengthDim.rCentre) * matBoundingRegion.height() +
                    matBoundingRegion.top()),
        QSizeF(
            (space.width / widthDim.rCentre) * matBoundingRegion.width(),
  ...
  for (const Drawing::ExtraAperture &aperture : drawing.extraApertures()) {
    QRectF apertureRegion(
        QPointF(
            (aperture.pos.x / widthDim.rCentre) * matBoundingRegion.width() +
                matBoundingRegion.left(),
            (aperture.pos.y / lengthDim.rCentre) * matBoundingRegion.height() +
                matBoundingRegion.top()),
        QSizeF((aperture.width / widthDim.rCentre) * matBoundingRegion.width(),
               (aperture.length / lengthDim.rCentre) *
                   matBoundingRegion.height()));
    painter.setBrush(QColor(141, 221, 247, 127));
    painter.drawRect(apertureRegion);

    painter.setBrush(QBrush(QColor(0, 0, 0, 255)));
    painter.drawText(apertureRegion, Qt::AlignHCenter | Qt::AlignVCenter,
                     aperture.aperture().apertureName().c_str());
  }
  for (const Drawing::DamBar &bar : drawing.damBars()) {
    QRectF padRegion(
        QPointF((bar.pos.x / widthDim.rCentre) * matBoundingRegion.width() +
                    matBoundingRegion.left(),
                (bar.pos.y / lengthDim.rCentre) * matBoundingRegion.height() +
                    matBoundingRegion.top()),
  ...
  for (const Drawing::CentreHole &hole : drawing.centreHoles()) {
    ...
    //         QPointF(matBoundingRegion.left() +
    //                 ((hole.pos.x -
    //                 DrawingComponentManager<Aperture>::findComponentByID(hole.apertureID).width
    //                 / 2) / widthDim.rCentre) * matBoundingRegion.width(),
    //                 matBoundingRegion.top() +
    //                 ((hole.pos.y -
    //                 DrawingComponentManager<Aperture>::findComponentByID(hole.apertureID).length
    //                 / 2) / lengthDim.rCentre) * matBoundingRegion.height()),
    //         QSizeF((DrawingComponentManager<Aperture>::findComponentByID(hole.apertureID).width
    //         / widthDim.rCentre) * matBoundingRegion.width(),
    //                (DrawingComponentManager<Aperture>::findComponentByID(hole.apertureID).length
    //                / lengthDim.rCentre) * matBoundingRegion.height())
    //);

    // painter.setPen(Qt::red);

    // if
    // (DrawingComponentManager<ApertureShape>::findComponentByID(DrawingComponentManager<Aperture>::findComponentByID(hole.apertureID).apertureShapeID).shape
    // != "DIA") {
    //     painter.drawRect(holeBounds);
    //     painter.setPen(smallDashPen);
    //     painter.drawLine(QPointF(holeBounds.center().x(), holeBounds.top()),
    //                      QPointF(holeBounds.center().x(),
    //                      holeBounds.bottom()));
    //     painter.drawLine(QPointF(holeBounds.left(), holeBounds.center().y()),
    //                      QPointF(holeBounds.right(),
    //                      holeBounds.center().y()));
    // } else {
    //     painter.setRenderHint(QPainter::Antialiasing);
    //     QPainterPath roundedRectPath;
    //     double radius =
    //             (std::min(DrawingComponentManager<Aperture>::findComponentByID(hole.apertureID).width,
    //             DrawingComponentManager<Aperture>::findComponentByID(hole.apertureID).length)
    //             / 2.0) / widthDim.rCentre * matBoundingRegion.width();
    ...
    QRectF verticalHoleBounds = QRectF(
        QPointF(
            matBoundingRegion.left() +
                ((hole.pos.x - hole.aperture().width / 2) / widthDim.rCentre) *
                    matBoundingRegion.width(),
            matBoundingRegion.top() +
                ((hole.pos.y - hole.aperture().length / 2) /
                 lengthDim.rCentre) *
                    matBoundingRegion.height()),
        QSizeF((hole.aperture().width / widthDim.rCentre) *
                   matBoundingRegion.width(),
               (hole.aperture().length / lengthDim.rCentre) *
                   matBoundingRegion.height()));
    QRectF horizontalHoleBounds = QRectF(
        QPointF(
            matBoundingRegion.left() +
                ((hole.pos.x - hole.aperture().length / 2) /
                 lengthDim.rCentre) *
                    matBoundingRegion.height(),
            matBoundingRegion.top() +
                ((hole.pos.y - hole.aperture().width / 2) / widthDim.rCentre) *
                    matBoundingRegion.width()),
        QSizeF((hole.aperture().length / lengthDim.rCentre) *
                   matBoundingRegion.height(),
               (hole.aperture().width / widthDim.rCentre) *
                   matBoundingRegion.width()));

    painter.setPen(Qt::red);
    ApertureShape shape =
        DrawingComponentManager<ApertureShape>::findComponentByID(
            hole.aperture().apertureShapeID);
    if (shape.shape == "SQ" || shape.shape == "SL") {
      painter.drawRect(verticalHoleBounds);
      painter.setPen(Qt::DashDotLine);
      painter.drawLine(
          QPointF(verticalHoleBounds.center().x(), verticalHoleBounds.top()),
          QPointF(verticalHoleBounds.center().x(),
                  verticalHoleBounds.bottom()));
      painter.drawLine(
          QPointF(verticalHoleBounds.left(), verticalHoleBounds.center().y()),
          QPointF(verticalHoleBounds.right(), verticalHoleBounds.center().y()));
    } else if (shape.shape == "ST") {
      painter.drawRect(horizontalHoleBounds);
      painter.setPen(Qt::DashDotLine);
      painter.drawLine(QPointF(horizontalHoleBounds.center().x(),
                               horizontalHoleBounds.top()),
                       QPointF(horizontalHoleBounds.center().x(),
                               horizontalHoleBounds.bottom()));
      painter.drawLine(QPointF(horizontalHoleBounds.left(),
                               horizontalHoleBounds.center().y()),
                       QPointF(horizontalHoleBounds.right(),
                               horizontalHoleBounds.center().y()));
    } else if (shape.shape == "DIA") {
      painter.setRenderHint(QPainter::Antialiasing);
      QPainterPath roundedRectPath;
      double radius =
          (std::min(hole.aperture().width, hole.aperture().length) / 2.0) /
          widthDim.rCentre * matBoundingRegion.width();
  ...
  for (const Drawing::Deflector &deflector : drawing.deflectors()) {
    ...
    deflectorBounds.moveTo(
        matBoundingRegion.left() +
            (deflector.pos.x / widthDim.rCentre) * matBoundingRegion.width(),
        matBoundingRegion.top() + (deflector.pos.y - deflector.size / root2) /
                                      lengthDim.rCentre *
                                      matBoundingRegion.height());
    deflectorBounds.lineTo(
        matBoundingRegion.left() + (deflector.pos.x + deflector.size / root2) /
                                       widthDim.rCentre *
                                       matBoundingRegion.width(),
        matBoundingRegion.top() +
            (deflector.pos.y / lengthDim.rCentre) * matBoundingRegion.height());
    deflectorBounds.lineTo(
        matBoundingRegion.left() +
            (deflector.pos.x / widthDim.rCentre) * matBoundingRegion.width(),
        matBoundingRegion.top() + (deflector.pos.y + deflector.size / root2) /
  ...
  for (const Drawing::Divertor &divertor : drawing.divertors()) {
    ...
    switch (divertor.side) {
      case Drawing::LEFT:
        divertorBounds.moveTo(
            matBoundingRegion.left(),
            matBoundingRegion.top() +
                (divertor.verticalPosition - divertor.width / (2 * root2)) /
                    lengthDim.rCentre * matBoundingRegion.height());
        divertorBounds.lineTo(
            matBoundingRegion.left() + (divertor.length / root2) /
                                           widthDim.rCentre *
                                           matBoundingRegion.width(),
            matBoundingRegion.top() +
                (divertor.verticalPosition - divertor.width / (2 * root2) +
                 divertor.length / root2) /
                    lengthDim.rCentre * matBoundingRegion.height());
        divertorBounds.lineTo(
            matBoundingRegion.left() + (divertor.length / root2) /
                                           widthDim.rCentre *
      ...
      case Drawing::RIGHT:
        divertorBounds.moveTo(
            matBoundingRegion.right(),
            matBoundingRegion.top() +
                (divertor.verticalPosition - divertor.width / (2 * root2)) /
                    lengthDim.rCentre * matBoundingRegion.height());
        divertorBounds.lineTo(
            matBoundingRegion.right() - (divertor.length / root2) /
                                            widthDim.rCentre *
                                            matBoundingRegion.width(),
            matBoundingRegion.top() +
                (divertor.verticalPosition - divertor.width / (2 * root2) +
...
void DrawingPDFWriter::drawArrow(QPainter &painter, QPointF from, QPointF to,
                                 const QString &label, ArrowMode mode,
                                 bool flipLabel, QPen tailPen, QPen headPen,
                                 double headSize, double angle,
                                 unsigned lines) {
  painter.save();
  ...
  if (!label.isEmpty()) {
    ...
    QRectF labelBounds = QFontMetricsF(painter.font())
                             .boundingRect(label)
    ...
    if (!flipLabel) {
      arrowToTextCentre.setLength(
          l * cos(qDegreesToRadians(abs(90 - theta) - alpha)));
    } else {
      arrowToTextCentre.setLength(
          -l * cos(qDegreesToRadians(abs(90 - theta) - alpha)));
    }
...
QRectF DrawingPDFWriter::adjustRect(const QRectF &rect) {
  return QRectF(rect);
  // return QRectF(rect.x() * 1.0504, rect.y() * 1.0143+ 30, rect.width()
  // * 1.0504, rect.height() * 1.0143 + 30);
}

void DrawingPDFWriter::drawText(
    QPainter &painter, QPointF start, const QString &label, unsigned lines,
    std::function<void(QRectF &, const QPointF &)> move) {
  const double padding = 10;
