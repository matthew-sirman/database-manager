...
void DatabaseResponseHandler::onMessageReceived(void*&& message, unsigned int messageSize) {
    ...
    case RequestType::SOURCE_PRODUCT_TABLE:
        DrawingComponentManager<Product>::sourceComponentTable(std::move(message), messageSize);
        break;
    case RequestType::SOURCE_BACKING_STRIPS_TABLE:
        DrawingComponentManager<BackingStrip>::sourceComponentTable(std::move(message), messageSize);
        break;
    case RequestType::SOURCE_APERTURE_TABLE:
        DrawingComponentManager<Aperture>::sourceComponentTable(std::move(message), messageSize);
        break;
    case RequestType::SOURCE_APERTURE_SHAPE_TABLE:
        DrawingComponentManager<ApertureShape>::sourceComponentTable(std::move(message), messageSize);
        break;
    case RequestType::SOURCE_MATERIAL_TABLE:
        DrawingComponentManager<Material>::sourceComponentTable(std::move(message), messageSize);
        break;
    case RequestType::SOURCE_EXTRA_PRICES_TABLE:
        DrawingComponentManager<ExtraPrice>::sourceComponentTable(std::move(message), messageSize);
        break;
    case RequestType::SOURCE_LABOUR_TIMES_TABLE:
        DrawingComponentManager<LabourTime>::sourceComponentTable(std::move(message), messageSize);
        break;
    case RequestType::SOURCE_POWDER_COATING_TABLE:
        DrawingComponentManager<PowderCoatingPrice>::sourceComponentTable(std::move(message), messageSize);
        break;
    case RequestType::SOURCE_SIDE_IRON_TABLE:
        DrawingComponentManager<SideIron>::sourceComponentTable(std::move(message), messageSize);
        break;
    case RequestType::SOURCE_SIDE_IRON_PRICES_TABLE:
        DrawingComponentManager<SideIronPrice>::sourceComponentTable(std::move(message), messageSize);
        break;
    case RequestType::SOURCE_MACHINE_TABLE:
        DrawingComponentManager<Machine>::sourceComponentTable(std::move(message), messageSize);
        break;
    case RequestType::SOURCE_MACHINE_DECK_TABLE:
        DrawingComponentManager<MachineDeck>::sourceComponentTable(std::move(message), messageSize);
        break;
    case RequestType::DRAWING_DETAILS:
        if (drawingReceivedCallback) {
            drawingReceivedCallback(DrawingRequest::deserialise(std::move(message)));
        }
        break;
    case RequestType::ADD_NEW_COMPONENT:
        if (addComponentCallback) {
            addComponentCallback(ComponentInsert::deserialise(std::move(message)).responseCode);
        }
        break;
    case RequestType::GET_NEXT_DRAWING_NUMBER:
        if (nextDrawingResponseCallback) {
            nextDrawingResponseCallback(NextDrawing::deserialise(std::move(message)));
        }
        break;
    case RequestType::CREATE_DATABASE_BACKUP:
        if (backupResponseCallback) {
            backupResponseCallback(DatabaseBackup::deserialise(std::move(message)).responseCode);
        }
        break;
    }
}
