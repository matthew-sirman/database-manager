...
void *DatabaseQuery::createBuffer(unsigned &size) const {
  ...
  // Serialise this object into the buffer. Note that this will call the
  // overridden serialise function, as the base function in DatabaseQuery is
  // pure virtual.
  serialise(buffer);
...
void DatabaseSearchQuery::serialise(void *target) const {
  // First we cast the buffer to a byte pointer so we can perform pointer
  // addition etc.
  unsigned char *buffer = (unsigned char *)target;
  // We first write the request type to the buffer. This is a search query, so
  // we write as such.
...
unsigned DatabaseSearchQuery::serialisedSize() const {
  ...
  // the RequestType for this search, as well as an unsigned for the
  // parameters specified in this search query
  unsigned size = sizeof(RequestType) + sizeof(unsigned);

  if (drawingNumber.has_value()) {
    // If we have a drawing number parameter, increment the size by the
    // space it will take.
    size += sizeof(unsigned char) + drawingNumber->size();
  }
  if (width.has_value()) {
    // If we have a width parameter, increment the size by the space it will
    // take.
    size += width->serialisedSize();
  }
  if (length.has_value()) {
    // If we have a length parameter, increment the size by the space it
    // will take.
    size += length->serialisedSize();
  }
  if (productType.has_value()) {
    // If we have a product type parameter, increment the size by the space
    // it will take.
    size += productType->serialisedSize();
  }
  if (numberOfBars.has_value()) {
    // If we have a number of bars parameter, increment the size by the
    // space it will take.
    size += sizeof(unsigned char);
  }
  if (aperture.has_value()) {
    // If we have an aperture parameter, increment the size by the space it
    // will take.
    size += aperture->serialisedSize();
  }
  if (topThickness.has_value()) {
    // If we have a top thickness parameter, increment the size by the space
    // it will take.
    size += topThickness->serialisedSize();
  }
  if (bottomThickness.has_value()) {
    // If we have a bottom thickness parameter, increment the size by the
    // space it will take.
    size += bottomThickness->serialisedSize();
  }
  if (dateRange.has_value()) {
    // If we have a date range parameter, increment the size by the space it
    // will take.
    size += dateRange->serialisedSize();
  }
  if (sideIronType.has_value()) {
    // If we have a side iron type parameter, increment the size by the
    // space it will take.
    size += sizeof(unsigned char);
  }
  if (sideIronLength.has_value()) {
    // If we have a side iron length parameter, increment the size by the
    // space it will take.
    size += sizeof(unsigned short);
  }
  if (sidelapMode.has_value()) {
    // If we have a sidelap mode parameter, increment the size by the space
    // it will take.
    size += sizeof(unsigned char);
  }
  if (overlapMode.has_value()) {
    // If we have an overlap mode parameter, increment the size by the space
    // it will take.
    size += sizeof(unsigned char);
  }
  if (sidelapWidth.has_value()) {
    // If we have a sidelap width parameter, increment the size by the space
    // it will take.
    size += sidelapWidth->serialisedSize();
  }
  if (overlapWidth.has_value()) {
    // If we have an overlap width parameter, increment the size by the
    // space it will take.
    size += overlapWidth->serialisedSize();
  }
  if (sidelapAttachment.has_value()) {
    // If we have a sidelap attachment parameter, increment the size by the
    // space it will take.
    size += sizeof(unsigned char);
  }
  if (overlapAttachment.has_value()) {
    // If we have an overlap attachment parameter, increment the size by the
    // space it will take.
    size += sizeof(unsigned char);
  }
  if (machine.has_value()) {
    // If we have a machine parameter, increment the size by the space it
    // will take.
  ...
  if (quantityOnDeck.has_value()) {
    // If we have a quantity on deck parameter, increment the size by the
    // space it will take.
    size += sizeof(unsigned char);
  }
  if (position.has_value()) {
    // If we have a position parameter, increment the size by the space it
    // will take.
    size += sizeof(unsigned char) + position->size();
  }
  if (machineDeck.has_value()) {
    // If we have a machine deck parameter, increment the size by the space
    // it will take.
...
DatabaseSearchQuery &DatabaseSearchQuery::deserialise(void *&&data) {
  ...
  if (searchParameters & (unsigned)SearchParameters::PRODUCT_TYPE) {
    query->productType = DrawingComponentManager<Product>::getComponentByHandle(
        Product::deserialise(buffer));
  ...
  if (searchParameters & (unsigned)SearchParameters::APERTURE) {
    query->aperture = DrawingComponentManager<Aperture>::getComponentByHandle(
        Aperture::deserialise(buffer));
  ...
  // If we have an overlap attachment specified, read it from the buffer in
  // the same way it was written. Otherwise, set this field to a nullopt.
  ...
  if (searchParameters & (unsigned)SearchParameters::MACHINE) {
    query->machine = DrawingComponentManager<Machine>::getComponentByHandle(
        Machine::deserialise(buffer));
  ...
  if (searchParameters & (unsigned)SearchParameters::MACHINE_DECK) {
    query->machineDeck =
        DrawingComponentManager<MachineDeck>::getComponentByHandle(
            MachineDeck::deserialise(buffer));
...
std::string DatabaseSearchQuery::toSQLQueryString() const {
  ...
  // First, we specify every value we wish to select. This includes two
  // aggregated fields, namely for the thicknesses and the laps, which are
  // returned as JSON arrays. Otherwise, every selection is a standard SQL
  // selection.
  sql << "SELECT d.mat_id AS mat_id, d.drawing_number AS drawing_number, "
         "d.width AS width, d.length AS length, "
         "mal.aperture_id AS aperture_id, "
         "(SELECT JSON_ARRAYAGG(t.material_thickness_id) FROM {0}.thickness "
         "AS t WHERE t.mat_id=d.mat_id) AS material_ids, "
         "d.tension_type AS tension_type, "
         "COALESCE((SELECT JSON_ARRAYAGG(JSON_ARRAY(l.type, l.width, "
         "l.mat_side)) "
         "FROM (SELECT mat_id, 'S' AS type, width, mat_side FROM "
         "{0}.sidelaps UNION "
         "SELECT mat_id, 'O' AS type, width, mat_side FROM {0}.overlaps) AS "
         "l "
         "WHERE l.mat_id=d.mat_id), JSON_ARRAY()) AS laps, "
         "COALESCE((SELECT JSON_ARRAYAGG(bs.bar_spacing) FROM "
         "{0}.bar_spacings AS bs WHERE bs.mat_id=d.mat_id), "
         "JSON_ARRAY()) AS spacings, "
         "COALESCE((SELECT JSON_ARRAYAGG(ea.aperture_id) FROM "
         "{0}.extra_apertures AS ea WHERE ea.mat_id = d.mat_id "
         "GROUP BY ea.mat_id),JSON_ARRAY()) AS ea "
      << std::endl;

  // We then specify the primary table to select from, and begin joining other
  // required tables.
  sql << "FROM {0}.drawings"
      << " AS d" << std::endl;
  sql << "INNER JOIN {0}.mat_aperture_link"
      << " AS mal ON d.mat_id=mal.mat_id" << std::endl;

  // Next we declare a vector of strings for the conditions. We do this
  // because, for example, we need to know if there are no conditions, because
  // in this case we do not put the "WHERE" clause at all in the query.
  std::vector<std::string> conditions;
  std::stringstream condition;

  // SELECT d.mat_id AS mat_id, d.drawing_number AS drawing_number, d.width AS
  // width, d.length AS length, mal.aperture_id AS aperture_id, (SELECT
  // JSON_ARRAYAGG(t.material_thickness_id) FROM thickness AS t WHERE
  // t.mat_id=d.mat_id) AS material_ids, d.tension_type AS tension_type,
  ...
  // SELECT mat_id, 'O' AS type, width, mat_side FROM overlaps) AS l WHERE
  // l.mat_id=d.mat_id), JSON_ARRAY()) AS laps, COALESCE((SELECT
  // JSON_ARRAYAGG(bs.bar_spacing) FROM bar_spacings AS bs WHERE
  // bs.mat_id=d.mat_id), JSON_ARRAY()) AS spacings.FROM drawings AS d.INNER
  // JOIN mat_aperture_link AS malON d.mat_id=mal.mat_id
  // SELECT
  //    d.mat_id AS mat_id,
  ...
  //    INNER JOIN screen_mat_database_dev.mat_aperture_link AS mal ON
  //    d.mat_id = mal.mat_id WHERE d.drawing_number LIKE 'EF%' GROUP BY
  //    d.mat_id,
  //    mal.aperture_id
  //    ORDER BY
  //    IF(
  ...
  // For each of these simpler queries, we simply check if the parameter is
  // present, and if it is, we create a conditional string matching the field
  // in the database to whatever was passed into the query.
  if (drawingNumber.has_value()) {
  ...
  if (width.has_value()) {
    condition.str(std::string());
    condition << "d.width BETWEEN " << width->lowerBound << " AND "
              << width->upperBound;
    conditions.push_back(condition.str());
  }
  if (length.has_value()) {
    condition.str(std::string());
    condition << "d.length BETWEEN " << length->lowerBound << " AND "
              << length->upperBound;
    conditions.push_back(condition.str());
  }
  if (productType.has_value()) {
  ...
  // The aperture query is marginally more complex, as we may need to specify
  // which direction it faces, but otherwise is very similar.
  if (aperture.has_value()) {
    condition.str(std::string());
  ...
  if (topThickness.has_value()) {
    condition.str(std::string());
    // If we just are searching for a top layer thickness, the search is the
    // same as the above conditions: a condition is simply added to match
    // the thickness ID
    if (!bottomThickness.has_value()) {
      sql << "INNER JOIN {0}.thickness"
          << " AS t ON d.mat_id=t.mat_id" << std::endl;
      condition << "t.material_thickness_id=" << topThickness->componentID();
    } else {
      // If the search is for multiple layers, it is more complex. We have
      // to join the table to itself on the same mat_id. Then, we impose
      // the restriction that t1's ID is less than t2. This avoids getting
      // duplicate search results. Finally, we check that the top layer
      // and bottom layer both match with what was searched. Note: As this
      // is a nested query, it is evaluated once and returns a set of
      // mat_id's which match the thickness restrictions. This could
      // potentially be slow, however the subquery itself is called a
      // minimal amount of times.
      condition << "d.mat_id IN (SELECT t1.mat_id AS mid FROM "
                   "{0}.thickness AS t1"
                << std::endl;
      condition << "INNER JOIN {0}.thickness AS t2 ON t1.mat_id=t2.mat_id"
                << std::endl;
      condition << "WHERE t1.thickness_id < t2.thickness_id AND" << std::endl;
      condition << "t1.material_thickness_id=" << topThickness->componentID()
                << " AND" << std::endl;
  ...
  if (dateRange.has_value()) {
    condition.str(std::string());
    condition << "d.drawing_date BETWEEN '"
              << dateRange->lowerBound.toMySQLDateString() << "' AND '"
              << dateRange->upperBound.toMySQLDateString() << "'";
    conditions.push_back(condition.str());
  }
  if (sideIronType.has_value() || sideIronLength.has_value()) {
    // For the side irons, we link to the side iron table itself and check
    // if the length and type align with what was searched.
    sql << "INNER JOIN {0}.mat_side_iron_link"
        << " AS msil ON d.mat_id=msil.mat_id" << std::endl;
    sql << "INNER JOIN {0}.side_irons"
        << " AS si ON msil.side_iron_id=si.side_iron_id" << std::endl;
    if (sideIronType.has_value()) {
      condition.str(std::string());
  ...
  if (sidelapMode.has_value() || sidelapWidth.has_value() ||
      sidelapAttachment.has_value()) {
    condition.str(std::string());
    // The sidelaps work by nesting a subquery which returns a set of all
    // mat_ids which match the criteria imposed by the sidelap parameters.
    condition << "d.mat_id IN (SELECT d_i.mat_id FROM {0}.drawings AS d_i"
              << std::endl;
    condition << "LEFT JOIN {0}.sidelaps AS s ON d_i.mat_id=s.mat_id"
              << std::endl;
    condition << "WHERE true" << std::endl;
    // First we impose that the width is in the correct range
    if (sidelapWidth.has_value()) {
      condition << "AND s.width BETWEEN " << sidelapWidth->lowerBound << " AND "
                << sidelapWidth->upperBound << std::endl;
    }
    ...
    // And finally, if we require that there are a specific number of laps
    // matching these criteria, we add a HAVING clause.
    if (sidelapMode.has_value()) {
  ...
  if (overlapMode.has_value() || overlapWidth.has_value() ||
      overlapAttachment.has_value()) {
    condition.str(std::string());
    // The overlaps work by nesting a subquery which returns a set of all
    // mat_ids which match the criteria imposed by the overlap parameters.
    condition << "d.mat_id IN (SELECT d_i.mat_id FROM {0}.drawings AS d_i"
              << std::endl;
    condition << "LEFT JOIN {0}.overlaps AS o ON d_i.mat_id=o.mat_id"
              << std::endl;
    condition << "WHERE true" << std::endl;
    // First we impose that the width is in the correct range
    if (overlapWidth.has_value()) {
      condition << "AND o.width BETWEEN " << overlapWidth->lowerBound << " AND "
                << overlapWidth->upperBound << std::endl;
    }
  ...
  if (machine.has_value() || manufacturer.has_value() ||
      quantityOnDeck.has_value() || position.has_value() ||
      machineDeck.has_value()) {
    // If the query requests that any restrictions concerning the machine
    // template should be imposed, we join to the machine templates table.
    sql << "INNER JOIN {0}.machine_templates AS mt ON "
           "d.template_id=mt.template_id"
        << std::endl;

    // For each of the restrictions, we simply check if the parameter
    // matches, if they are included in the query parameters.
    if (machine.has_value()) {
      condition.str(std::string());
    ...
    if (manufacturer.has_value()) {
      sql << "INNER JOIN {0}.machines AS m ON mt.machine_id=m.machine_id"
          << std::endl;
      condition.str(std::string());
  ...
  // We include condition always as error avoidance, so that if a material is
  // missing, it is removed from the results as opposed to causing an error
  // and blocking all results.
  // sql << "WHERE " << std::endl;
  ...
  // Finally, we group the results by mat_id and aperture_id to aggregate
  // them, and then order the results by the drawing number order. This
  // ordering is such that all drawings with a single letter (e.g A34) appear
  // before double letter drawings (e.g. EA34) and then the drawings are order
  // lexicographically in descending order so more recent drawings appear
  // higher up the search list.
...
std::vector<DrawingSummary> DatabaseSearchQuery::getQueryResultSummaries(
    mysqlx::RowResult resultSet, std::ostream *errStream) {
  ...
  for (const mysqlx::Row &row : resultSet) {
    try {
      ...
      if (row[4].isNull()) {
        lockLog {
          *ss << "Missing aperture on drawing "
                                 << summary.drawingNumber << std::endl;
          Logger::logError();
        }
        continue;
      }
      // It may be the case that this mat uses a bidirectional aperture,
      // in which case we return all matching apertures and choose the
      // correct one, based upon the aperture direction
      ...
      // We nullify the thickness handles and lap sizes to clear any
      // residual memory which might be accidentally written
      ...
      if (row[5].isNull()) {
        lockLog {
          *ss << " SQL Error: Missing material on drawing "
                                 << summary.drawingNumber << std::endl;
          Logger::logError();
        }
        continue;
      }
      // We then loop through each returned element in the thicknesses
      // returned and set the corresponding thickness handle in the
      // summary object
      for (unsigned thicknessSlot = 0; thicknessSlot < row[5].elementCount();
           thicknessSlot++) {
        summary.thicknessHandles[thicknessSlot] =
      ...
      for (const mysqlx::Value &lap : row[7]) {
        // If we encounter a "damaged" lap, we just skip it to avoid a
        // possible error
        if (lap[2].isNull()) {
          continue;
        }

        // We calculate the index this lap belongs to. This depends on
        // the side of the mat, the tension type and whether it is a
        // sidelap or overlap. Note that a boolean equivalence will
        // return 1 if equal and 0 if inequal, so if lap[2] == "Right",
        // then the index will be 1 to start with, otherwise 0. This
        // means we can easily calculate the index associated with this
        // lap value.
        ...
        // Finally, after computing the index, we simply write the size
        // to the summary in the correct slot.
      ...
      for (unsigned aperture : row[9]) {
        summary.addExtraAperture(
            DrawingComponentManager<Aperture>::findComponentByID(aperture)
                .handle());
      }

      // Once we have finished creating this summary, we add it to the
      // list
...
DrawingRequest &DrawingRequest::makeRequest(unsigned matID,
  ...
  // We set the drawing data to be a nullopt because this is the request; we
  // do not currently know what the drawing data contains
...
void DrawingRequest::serialise(void *target) const {
  ...
  *((unsigned *)buffer) = responseEchoCode;
  ...
  // We set a simple boolean which determines whether or not the request
  // contains a drawing or not (i.e. if it is a request or a response)
  bool hasDrawingData = drawingData.has_value();
  *buffer++ = hasDrawingData;

  // If we do have a drawing to write into the buffer, we serialise it using
  // the DrawingSerialiser into the buffer
  if (hasDrawingData) {
    DrawingSerialiser::serialise(drawingData.value(), buffer);
  }
}

// Helper function to return the amount of space this object will occupy in a
// buffer
unsigned int DrawingRequest::serialisedSize() const {
  // Returns the size of the header info, the sizes of the matID and response
  // code, the size of the byte used to determine whether there is a drawing,
  // and then if there is a drawing, the size this drawing will occupy.
  return sizeof(RequestType) + sizeof(unsigned) + sizeof(unsigned) +
         sizeof(unsigned char) +
         (drawingData.has_value()
              ? DrawingSerialiser::serialisedSize(drawingData.value())
              : 0);
}
...
DrawingRequest &DrawingRequest::deserialise(void *&&data) {
  ...
  if (hasDrawingData) {
    // If there is a drawing in this message, we deserialise it into the
    // drawingData optional value
    drawingRequest->drawingData = DrawingSerialiser::deserialise(buffer);
  } else {
    // Otherwise we set the drawingData to a nullopt, indicating that it is
    // not present
...
unsigned int DrawingInsert::serialisedSize() const {
  // Returns the size of the header info, the sizes of the insert response and
  // echo codes, the size of the byte used to determine if it is in forcing
  // mode, the size of the byte used to determine whether there is a drawing,
  // and then if there is a drawing, the size this drawing will occupy.
  return sizeof(RequestType) + sizeof(InsertResponseCode) + sizeof(unsigned) +
...
DrawingInsert &DrawingInsert::deserialise(void *&&data) {
  ...
  // Next we cast the data buffer to a byte buffer and increment by the size
  // of a request type, as we already know what the request type is
  unsigned char *buffer = (unsigned char *)data + sizeof(RequestType);
  ...
  if (hasDrawingData) {
    // If there is an attached drawing, we deserialise it into the drawing
    // data object
...
void DrawingInsert::setForce(bool val) { force = val; }

// Getter for forcing mode
bool DrawingInsert::forcing() const { return force; }
...
std::string DrawingInsert::drawingInsertQuery(unsigned templateID) const {
  ...
  insert << "(drawing_number, product_id, template_id, width, length, "
            "tension_type, drawing_date, no_of_bars, "
            "rebated, hyperlink, notes)"
         << std::endl;
  insert << "VALUES" << std::endl;

  // Next we add the simple values from the drawingData object
  insert << "('" << drawingData->drawingNumber() << "', "
         << drawingData->product().componentID() << ", " << templateID << ", "
         << drawingData->width() << ", " << drawingData->length() << ", ";

  // The tension type needs to be converted from an enum type to a string, so
  // we use a simple switch statement
  ...
  insert << ", '" << drawingData->date().toMySQLDateString() << "', "
         << drawingData->numberOfBars() << ", " << drawingData->rebated()
         << ", '" << drawingData->hyperlink().generic_string() << "', '"
...
// Creates a MySQL query string for inserting the bars into the bar_spacings
// table
std::string DrawingInsert::barSpacingInsertQuery(unsigned matID) const {
  // First we check if there are no bars. If there are none, we have nothing
  // to enter so we just return an empty string.
  ...
  for (unsigned i = 0; i < drawingData->numberOfBars(); i++) {
    insert << "(" << matID << ", " << drawingData->barSpacing(i) << ", "
           << drawingData->barWidth(i + 1) << ", " << i << ")";
...
// Creates a MySQL query string for inserting the machine data into the
...
// Creates a MySQL query string for testing if the machine template already
// exists
std::string DrawingInsert::testMachineTemplateQuery() const {
  ...
  query << "WHERE machine_id=" << t.machine().componentID() << " AND "
        << std::endl;
...
// Creates a MySQL query string for inserting the aperture into the
// mat_aperture_link table
...
// Creates a MySQL query string for inserting the side irons into the
// mat_side_iron_link table
std::string DrawingInsert::sideIronInsertQuery(unsigned matID) const {
  std::stringstream insert;

  // First specify the columns to insert
  insert << "INSERT INTO {0}.mat_side_iron_link" << std::endl;
  insert << "(mat_id, side_iron_id, bar_width, inverted, cut_down, "
            "side_iron_index, fixed_end, feed_end, hook_orientation, "
            "strap_id)"
         << std::endl;
  insert << "VALUES" << std::endl;

  // Then add the data to insert for both of the side iron side. We specify
  // which is which by the final index parameter.
  insert << "(" << matID << ", ";
  insert << drawingData->sideIron(Drawing::LEFT).componentID() << ", ";
  insert << drawingData->leftMargin() << ", ";
  insert << drawingData->sideIronInverted(Drawing::LEFT) << ", ";
  insert << drawingData->sideIronCutDown(Drawing::LEFT) << ", 0, ";
  insert << (drawingData->sideIronFixedEnd(Drawing::LEFT).has_value()
                 ? std::to_string(
                       drawingData->sideIronFixedEnd(Drawing::LEFT).value())
                 : "NULL");
  insert << ", ";
  insert << (drawingData->sideIronFeedEnd().has_value() &&
                     drawingData->sideIronFeedEnd().value() == Drawing::LEFT
                 ? "1"
                 : "0");
  insert << ", ";
  insert
      << (drawingData->sideIronHookOrientation(Drawing::LEFT).has_value()
              ? std::to_string(
                    drawingData->sideIronHookOrientation(Drawing::LEFT).value())
              : "NULL");
  insert << ", ";
  insert << (drawingData->hasStrap(Drawing::LEFT)
                 ? std::to_string(
                       drawingData->sideIronStrap(Drawing::LEFT).componentID())
                 : "NULL");
  insert << "), " << std::endl;
  insert << "(" << matID << ", "
         << drawingData->sideIron(Drawing::RIGHT).componentID() << ", "
         << drawingData->rightMargin() << ", "
         << drawingData->sideIronInverted(Drawing::RIGHT) << ", "
         << drawingData->sideIronCutDown(Drawing::RIGHT) << ", 1,"
         << (drawingData->sideIronFixedEnd(Drawing::RIGHT).has_value()
                 ? std::to_string(
                       drawingData->sideIronFixedEnd(Drawing::RIGHT).value())
                 : "NULL")
         << ", "
         << (drawingData->sideIronFeedEnd().has_value() &&
                     drawingData->sideIronFeedEnd().value() == Drawing::RIGHT
                 ? "1"
                 : "0")
         << ", "
         << (drawingData->sideIronHookOrientation(Drawing::RIGHT).has_value()
                 ? std::to_string(
                       drawingData->sideIronHookOrientation(Drawing::RIGHT)
                           .value())
                 : "NULL")
         << ", "
         << (drawingData->hasStrap(Drawing::RIGHT)
                 ? std::to_string(
                       drawingData->sideIronStrap(Drawing::RIGHT).componentID())
                 : "NULL")
         << ")" << std::endl;
...
// Creates a MySQL query string for inserting the materials into the thickness
// table
std::string DrawingInsert::thicknessInsertQuery(unsigned matID) const {
  ...
  insert << "(" << matID << ", "
         << drawingData->material(Drawing::TOP)->componentID() << ")";

  // Next we add the option bottom material, if it exists. Not every mat will
  // have this
  if (drawingData->material(Drawing::BOTTOM).has_value()) {
    insert << ", " << std::endl
           << "(" << matID << ", "
           << drawingData->material(Drawing::BOTTOM)->componentID() << ")";
  }
...
// Creates a MySQL query string for inserting the overlaps into the overlaps
// table
std::string DrawingInsert::overlapsInsertQuery(unsigned matID) const {
  // First we check if there are no overlaps. If there are none, we have
  // nothing to enter so we just return an empty string.
  ...
  insert << "(mat_id, width, mat_side, attachment_type, material_id)"
         << std::endl;
  insert << "VALUES" << std::endl;

  // We get the left and right overlaps from the drawing data. If they do not
  // exist, these will be nullopts.
  std::optional<Drawing::Lap> left = drawingData->overlap(Drawing::LEFT),
                              right = drawingData->overlap(Drawing::RIGHT);
...
// Creates a MySQL query string for inserting the sidelaps into the sidelaps
// table
std::string DrawingInsert::sidelapsInsertQuery(unsigned matID) const {
  // First we check if there are no sidelaps. If there are none, we have
  // nothing to enter so we just return an empty string.
  ...
  // We get the left and right sidelaps from the drawing data. If they do not
  // exist, these will be nullopts.
  std::optional<Drawing::Lap> left = drawingData->sidelap(Drawing::LEFT),
                              right = drawingData->sidelap(Drawing::RIGHT);
...
// Creates a MySQL query string for inserting the punch program PDF hyperlinks
// into the punch_program_pdfs table
std::string DrawingInsert::punchProgramsInsertQuery(unsigned matID) const {
  std::vector<std::filesystem::path> punchPDFs =
      drawingData->pressDrawingHyperlinks();

  // First we check if there are no PDFs. If there are none, we have nothing
  // to enter so we just return an empty string.
  if (punchPDFs.empty()) {
    return std::string();
  }
  ...
  // Next we loop over each PDF and append it to the values we add in the
  // query.
  for (std::vector<std::filesystem::path>::const_iterator it =
           punchPDFs.begin();
       it != punchPDFs.end(); it++) {
...
// Creates a MySQL query string for inserting Impact Pads into the corresponding
// table
std::string DrawingInsert::impactPadsInsertQuery(unsigned matID) const {
  ...
  insert
      << "(mat_id, material_id, aperture_id, width, length, x_coord, y_coord)"
      << std::endl;
  insert << "VALUES" << std::endl;

  for (std::vector<Drawing::ImpactPad>::const_iterator it = impactPads.begin();
       it != impactPads.end(); it++) {
    Drawing::ImpactPad pad = *it;

    Aperture &ap = pad.aperture();

    insert << "(" << matID << ", " << pad.material().componentID() << ", "
           << ap.componentID() << ", ";

    insert << pad.width << ", " << pad.length << ", " << pad.pos.x << ", "
           << pad.pos.y << ")";
...
std::string DrawingInsert::damBarInsertQuery(unsigned matID) const {
  ...
  insert << "(mat_id, width, length, x_coord, y_coord, material_id)"
         << std::endl;
  insert << "VALUES" << std::endl;

  for (std::vector<Drawing::DamBar>::const_iterator it = damBars.begin();
       it != damBars.end(); it++) {
    Drawing::DamBar bar = *it;

    insert << "(" << matID << ", " << bar.width << ", " << bar.length << ","
           << bar.pos.x << ", " << bar.pos.y << ", "
           << bar.material().componentID() << ")";
...
std::string DrawingInsert::blankSpaceInsertQuery(unsigned matID) const {
  ...
  for (std::vector<Drawing::BlankSpace>::const_iterator it =
           blankSpaces.begin();
       it != blankSpaces.end(); it++) {
    Drawing::BlankSpace space = *it;

    insert << "(" << matID << ", " << space.width << ", " << space.length
...
std::string DrawingInsert::extraApertureInsertQuery(unsigned matID) const {
  std::vector<Drawing::ExtraAperture> extraApertures =
      drawingData->extraApertures();
  ...
  insert << "(mat_id, width, length, x_coord, y_coord, aperture_id)"
         << std::endl;
  insert << "VALUES" << std::endl;

  for (std::vector<Drawing::ExtraAperture>::const_iterator it =
           extraApertures.begin();
       it != extraApertures.end(); it++) {
    Drawing::ExtraAperture aperture = *it;

    insert << "(" << matID << ", " << aperture.width << ", " << aperture.length
           << ", " << aperture.pos.x << ", " << aperture.pos.y << ", "
           << aperture.aperture().componentID() << ")";
...
std::string DrawingInsert::centreHolesInsertQuery(unsigned matID) const {
  const std::vector<Drawing::CentreHole> &centreHoles =
      drawingData->centreHoles();
  ...
  for (std::vector<Drawing::CentreHole>::const_iterator it =
           centreHoles.begin();
       it != centreHoles.end(); it++) {
    Drawing::CentreHole hole = *it;

    insert << "(" << matID << ", " << hole.pos.x << ", " << hole.pos.y << ", "
           << hole.aperture().componentID() << ")";
...
std::string DrawingInsert::deflectorsInsertQuery(unsigned matID) const {
  ...
  for (std::vector<Drawing::Deflector>::const_iterator it = deflectors.begin();
       it != deflectors.end(); it++) {
    Drawing::Deflector deflector = *it;

    insert << "(" << matID << ", " << deflector.material().componentID() << ", "
           << deflector.size << ", " << deflector.pos.x << ", "
           << deflector.pos.y << ")";
...
std::string DrawingInsert::divertorsInsertQuery(unsigned matID) const {
  ...
  insert << "(mat_id, material_id, width, length, mat_side, y_coord)"
         << std::endl;
  insert << "VALUES" << std::endl;

  for (std::vector<Drawing::Divertor>::const_iterator it = divertors.begin();
       it != divertors.end(); it++) {
    Drawing::Divertor divertor = *it;

    insert << "(" << matID << ", " << divertor.material().componentID() << ", "
           << divertor.width << ", " << divertor.length << ", '";
  ...
  switch (insertType) {
    ...
    case InsertType::APERTURE:
      ...
      *((unsigned *)buff) = apertureData->shapeID;
      buff += sizeof(unsigned);
      *buff++ = apertureData->nibbleApertureId.has_value();
      if (apertureData->nibbleApertureId.has_value()) {
        *((unsigned *)buff) = apertureData->nibbleApertureId.value();
      }
      break;
    ...
    case InsertType::SIDE_IRON: {
      ...
      *((unsigned *)buff) = sideIronData->length;
      ...
      unsigned char drawingNumberSize = sideIronData->drawingNumber.size(),
                    hyperlinkSize =
                        sideIronData->hyperlink.generic_string().size();
      *buff++ = drawingNumberSize;
      memcpy(buff, sideIronData->drawingNumber.c_str(), drawingNumberSize);
      buff += drawingNumberSize;
      *buff++ = hyperlinkSize;
      memcpy(buff, sideIronData->hyperlink.generic_string().c_str(),
             hyperlinkSize);
      buff += hyperlinkSize;
      *buff++ = sideIronData->extraflex;
    ...
    case (InsertType::EXTRA_PRICE): {
      ...
      *((float *)buff) = extraPriceData->price;
      buff += sizeof(float);
      buff += extraPriceData->amount.has_value();
      if (extraPriceData->amount.has_value()) {
        *((unsigned *)buff) = extraPriceData->amount.value();
        buff += sizeof(unsigned);
      }
      buff += extraPriceData->squareMetres.has_value();
      if (extraPriceData->squareMetres.has_value()) {
        *((float *)buff) = extraPriceData->squareMetres.value();
        buff += sizeof(float);
      }
    ...
    case (InsertType::STRAP): {
      *((unsigned *)buff) = strapData->materialHandle;
      buff += sizeof(unsigned);
      *buff++ = strapData->isWTL;
      break;
    }
  }
}
...
unsigned int ComponentInsert::serialisedSize() const {
  switch (insertType) {
    case InsertType::APERTURE:
      return sizeof(RequestType) + sizeof(InsertType) +
             sizeof(ComponentInsertResponse) + apertureData->serialisedSize();
    case InsertType::MACHINE:
      return sizeof(RequestType) + sizeof(InsertType) +
             sizeof(ComponentInsertResponse) + machineData->serialisedSize();
    case InsertType::SIDE_IRON:
      return sizeof(RequestType) + sizeof(InsertType) +
             sizeof(ComponentInsertResponse) + sideIronData->serialisedSize();
    case InsertType::SIDE_IRON_PRICE:
      return sizeof(RequestType) + sizeof(InsertType) +
             sizeof(ComponentInsertResponse) +
             sideIronPriceData->serialisedSize();
    case InsertType::MATERIAL:
      return sizeof(RequestType) + sizeof(InsertType) +
             sizeof(ComponentInsertResponse) + materialData->serialisedSize();
    case InsertType::MATERIAL_PRICE:
      return sizeof(RequestType) + sizeof(InsertType) +
             sizeof(ComponentInsertResponse) +
             materialPriceData->serialisedSize();
    case InsertType::EXTRA_PRICE:
      return sizeof(RequestType) + sizeof(InsertType) +
             sizeof(ComponentInsertResponse) + extraPriceData->serialisedSize();
    case InsertType::LABOUR_TIMES:
      return sizeof(RequestType) + sizeof(InsertType) +
             sizeof(ComponentInsertResponse) + labourTimeData->serialisedSize();
    case InsertType::SPECIFIC_SIDE_IRON_PRICE:
      return sizeof(RequestType) + sizeof(InsertType) +
             sizeof(ComponentInsertResponse) +
             specificSideIronPriceData->serialisedSize();
    case InsertType::POWDER_COATING:
      return sizeof(RequestType) + sizeof(InsertType) +
             sizeof(ComponentInsertResponse) +
             powderCoatingPriceData->serialisedSize();
    case InsertType::STRAP:
      return sizeof(RequestType) + sizeof(InsertType) +
             sizeof(ComponentInsertResponse) + strapData->serialisedSize();
    case InsertType::NONE:
    default:
      return sizeof(RequestType) + sizeof(InsertType) +
             sizeof(ComponentInsertResponse);
...
ComponentInsert &ComponentInsert::deserialise(void *&&data) {
  ...
  switch (insert->insertType) {
    ...
    case InsertType::APERTURE: {
      ...
      bool isNibble = *buff++;
      if (isNibble) {
        data.nibbleApertureId = std::make_optional(*((unsigned *)buff));
        buff += sizeof(unsigned);
      }
    ...
    case InsertType::STRAP: {
      StrapData data;
      data.materialHandle = *((unsigned *)buff);
      buff += sizeof(unsigned);
      data.isWTL = *buff++;
      insert->strapData = data;
      break;
    };
    case InsertType::EXTRA_PRICE: {
      ExtraPriceData data;
      ...
      bool hasAmount = *buff++;
      if (hasAmount) {
        data.amount = std::make_optional(*((unsigned*)buff));
        buff += sizeof(unsigned);
      }
      bool hasSqM = *buff++;
      if (hasSqM) {
        data.squareMetres= std::make_optional(*((float*)buff));
        buff += sizeof(float);
      }
...
/// <summary>
/// Makes this ComponentInsert based upon an ApertureData.
/// </summary>
/// <param name="data">The data to base the ComponentInsert on.</param>
template <>
void ComponentInsert::setComponentData(ApertureData &&data) {
...
/// <summary>
/// Makes this ComponentInsert based upon an MachineData.
/// </summary>
/// <param name="data">The data to base the ComponentInsert on.</param>
template <>
void ComponentInsert::setComponentData(MachineData &&data) {
...
/// <summary>
/// Makes this ComponentInsert based upon an SideIronData.
/// </summary>
/// <param name="data">The data to base the ComponentInsert on.</param>
template <>
void ComponentInsert::setComponentData(SideIronData &&data) {
...
/// <summary>
/// Makes this ComponentInsert based upon an SpecificSideIronPriceData.
/// </summary>
/// <param name="data">The data to base the ComponentInsert on.</param>
template <>
void ComponentInsert::setComponentData(SpecificSideIronPriceData &&data) {
...
/// <summary>
/// Makes this ComponentInsert based upon an PowderCoatingPriceData.
/// </summary>
/// <param name="data">The data to base the ComponentInsert on.</param>
template <>
void ComponentInsert::setComponentData(PowderCoatingPriceData &&data) {
...
/// <summary>
/// Makes this ComponentInsert based upon an SideIronPriceData.
/// </summary>
/// <param name="data">The data to base the ComponentInsert on.</param>
template <>
void ComponentInsert::setComponentData(SideIronPriceData &&data) {
...
/// <summary>
/// Makes this ComponentInsert based upon an MaterialData.
/// </summary>
/// <param name="data">The data to base the ComponentInsert on.</param>
template <>
void ComponentInsert::setComponentData(MaterialData &&data) {
...
/// <summary>
/// Makes this ComponentInsert based upon an MaterialPriceData.
/// </summary>
/// <param name="data">The data to base the ComponentInsert on.</param>
template <>
void ComponentInsert::setComponentData(MaterialPriceData &&data) {
...
/// <summary>
/// Makes this ComponentInsert based upon an ExtraPriceData.
/// </summary>
/// <param name="data">The data to base the ComponentInsert on.</param>
template <>
void ComponentInsert::setComponentData(ExtraPriceData &&data) {
...
/// <summary>
/// Makes this ComponentInsert based upon an LabourTimeData.
/// </summary>
/// <param name="data">The data to base the ComponentInsert on.</param>
template <>
void ComponentInsert::setComponentData(LabourTimeData &&data) {
...
/// <summary>
/// Makes this ComponentInsert based upon a StrapData.
/// </summary>
/// <param name="data">The data to base the ComponentInsert on.</param>
template <>
void ComponentInsert::setComponentData(StrapData &&data) {
  insertType = InsertType::STRAP;
  responseCode = ComponentInsertResponse::NONE;
...
std::string ComponentInsert::toSQLQueryString() const {
  ...
  switch (insertType) {
    case InsertType::APERTURE:
      insert << "INSERT INTO {0}.apertures (width, length, base_width, "
                "base_length, quantity, shape_id, nibble, "
                "nibble_aperture_id)"
             << std::endl;
      insert << "VALUES" << std::endl;
      insert << "(" << apertureData->width << ", " << apertureData->length
             << ", " << apertureData->baseWidth << ", "
             << apertureData->baseLength << ", " << apertureData->quantity
             << ", " << apertureData->shapeID << ", "
             << apertureData->nibbleApertureId.has_value() << ","
             << (apertureData->nibbleApertureId.has_value()
                     ? std::to_string(apertureData->nibbleApertureId.value())
                     : "NULL")
             << ")" << std::endl;
      break;
    case InsertType::MACHINE:
      ...
      insert << "VALUES" << std::endl;
      insert << "('" << machineData->manufacturer << "', '"
             << machineData->model << "')" << std::endl;
      break;
    case InsertType::STRAP:
      insert << "INSERT INTO {0}.straps (material_id, is_wtl)" << std::endl;
      insert << "VALUES" << std::endl;
      insert << "("
             << DrawingComponentManager<Material>::getComponentByHandle(
                    strapData->materialHandle)
                    .componentID()
             << "," << strapData->isWTL << ")" << std::endl;
      break;
    case InsertType::SIDE_IRON:
      insert << "INSERT INTO {0}.side_irons (type, length, "
                "drawing_number, hyperlink, price, screws)"
             << std::endl;
      insert << "VALUES" << std::endl;
      insert << "(" << (unsigned)sideIronData->type << ", "
             << sideIronData->length << ", '" << sideIronData->drawingNumber
             << "', '" << sideIronData->hyperlink.generic_string() << "', "
             << (sideIronData->price.has_value()
                     ? std::to_string(*sideIronData->price)
                     : "NULL")
             << ", "
             << (sideIronData->screws.has_value()
                     ? std::to_string(*sideIronData->screws)
                     : "NULL")
             << ")" << std::endl;
      break;
    case InsertType::SIDE_IRON_PRICE:
      switch (sideIronPriceData->priceMode) {
        case PriceMode::ADD:
          insert << "INSERT INTO {0}.side_iron_prices (type, "
                    "lower_length, upper_length, product_id, price) "
                    "VALUES ";
          insert << "(" << (unsigned)sideIronPriceData->type << " , "
                 << sideIronPriceData->lowerLength << " , "
                 << sideIronPriceData->upperLength << " , ";
          insert << sideIronPriceData->extraflex + 1 << " , "
                 << sideIronPriceData->price << ")";
          break;
        case PriceMode::UPDATE:
          if (!sideIronPriceData->componentID.has_value()) {
            std::cout << "cannot update side iron data as "
                         "component id is missing";
          }

          insert << "UPDATE {0}.side_iron_prices SET" << std::endl;
          insert << "type=" << (unsigned)sideIronPriceData->type
                 << " , lower_length=" << sideIronPriceData->lowerLength
                 << " , upper_length=" << sideIronPriceData->upperLength;
          insert << " , product_id=" << sideIronPriceData->extraflex
                 << " , price=" << sideIronPriceData->price;
          insert << " WHERE side_iron_price_id = "
                 << *sideIronPriceData->componentID << std::endl;
          break;
        case PriceMode::REMOVE:
          if (!sideIronPriceData->componentID.has_value()) {
            std::cout << "cannot remove side iron data as "
                         "component id is missing";
          }
      ...
      break;
    case InsertType::MATERIAL:
      insert << "INSERT INTO {0}.materials (material, hardness, thickness)"
             << std::endl;
      insert << "VALUES" << std::endl;
      insert << "('" << materialData->materialName << "', "
             << materialData->hardness << ", " << materialData->thickness << ")"
             << std::endl;
      break;
    case InsertType::MATERIAL_PRICE:
      switch (materialPriceData->priceMode) {
        case (PriceMode::ADD):
          insert << "INSERT INTO {0}.material_prices (material_id, "
                    "width, length, price, pricing)"
                 << std::endl;
          insert << "VALUES" << std::endl;
          if (materialPriceData->pricingType ==
              MaterialPricingType::RUNNING_M) {
            insert << "(" << materialPriceData->material_id << ", "
                   << materialPriceData->width << ", "
                   << materialPriceData->length << ", "
                   << materialPriceData->price << ", 'running_m')" << std::endl;
          } else if (materialPriceData->pricingType ==
                     MaterialPricingType::SQUARE_M) {
            insert << "(" << materialPriceData->material_id << ", "
                   << materialPriceData->width << ", "
                   << materialPriceData->length << ", "
                   << materialPriceData->price << ", 'square_m')" << std::endl;
          } else if (materialPriceData->pricingType ==
                     MaterialPricingType::SHEET) {
            insert << "(" << materialPriceData->material_id << ", "
                   << materialPriceData->width << ", "
                   << materialPriceData->length << ", "
                   << materialPriceData->price << ", 'sheet')" << std::endl;
          }
          break;
        ...
        case (PriceMode::UPDATE):
          if (materialPriceData->pricingType ==
            ...
            insert << "width = " << materialPriceData->width
                   << ", length = " << materialPriceData->length
                   << ", price = " << materialPriceData->price << ", pricing = "
                   << "'running_m'" << std::endl;
            insert << "WHERE material_price_id = "
                   << materialPriceData->material_price_id << std::endl;
          } else if (materialPriceData->pricingType ==
                     MaterialPricingType::SQUARE_M) {
            insert << "UPDATE {0}.material_prices SET" << std::endl;
            insert << "width = " << materialPriceData->width
    ...
    case InsertType::EXTRA_PRICE:
      ...
      insert << "price = " << extraPriceData->price;
      if (extraPriceData->amount.has_value()) {
        insert << ", amount = " << extraPriceData->amount.value();
      }
      if (extraPriceData->squareMetres.has_value()) {
        insert << ", square_metres = " << extraPriceData->squareMetres.value();
      }
      insert << std::endl;
      insert << "WHERE" << std::endl;
      insert << "price_id = " << extraPriceData->priceId;
      insert << " AND type = '" << ExtraPrice::getName.at(extraPriceData->type)
             << "'";
      break;
    case InsertType::LABOUR_TIMES:
    ...
    case InsertType::SPECIFIC_SIDE_IRON_PRICE:
      switch (specificSideIronPriceData->priceMode) {
        case PriceMode::UPDATE: {
          ...
          insert << " WHERE" << std::endl;
          insert << "side_iron_id = "
                 << specificSideIronPriceData->sideIronComponentID;
          break;
        }
        case PriceMode::REMOVE: {
          insert << "UPDATE {0}.side_irons set" << std::endl;
...
void ComponentInsert::clearComponentData() { insertType = InsertType::NONE; }
...
unsigned int DatabaseBackup::serialisedSize() const {
  return sizeof(RequestType) + sizeof(BackupResponse) + sizeof(unsigned char) +
         backupName.size();
}
...
unsigned int NextDrawing::serialisedSize() const {
  return sizeof(RequestType) + sizeof(DrawingType) + sizeof(unsigned char) +
         (drawingNumber.has_value()
